...
def run_ten_archetype_system():
    dfs = load_data()
    print("Preloaded all data.")
    
    # 7 Core Order Flow & CVD Archetypes
    archetypes = [
        # 1. Dual-Brain Standard
        {"name": "A1_DualBrain_Std", "sig_fn": lambda df: (
            ((df['p200'] > -0.5) & (df['mc'] > 0) & (df['p8'] < -0.16) & (df['zc20'] > df['zb20'] - 0.08)) |
            ((df['trend_strength'] < 0.45) & (df['liq_long_ratio'] > 1.4) & (df['p8'] < -0.15) & (df['rsi'] < 38)),
            ((df['p200'] < 0.5) & (df['mc'] < 0) & (df['p8'] > 0.16) & (df['zc20'] < df['zb20'] + 0.08)) |
            ((df['trend_strength'] < 0.45) & (df['liq_short_ratio'] > 1.4) & (df['p8'] > 0.15) & (df['rsi'] > 62))
        )},
        # 2. Deep Squeeze & Liquidation
        {"name": "A2_DeepSqueeze", "sig_fn": lambda df: (
            ((df['mc'] > 0) & (df['p8'] < -0.22) & (df['zc20'] > df['zb20'] - 0.05)) |
            ((df['liq_long_ratio'] > 1.6) & (df['rsi'] < 35) & (df['zc4'] > -0.5)),
            ((df['mc'] < 0) & (df['p8'] > 0.22) & (df['zc20'] < df['zb20'] + 0.05)) |
            ((df['liq_short_ratio'] > 1.6) & (df['rsi'] > 65) & (df['zc4'] < 0.5))
        )},
        # 3. Consolidation Funding & Orderflow Squeeze
        {"name": "A3_FundingSqueeze", "sig_fn": lambda df: (
            ((df['trend_strength'] < 0.40) & (df['rsi'] < 33) & (df['p8'] < -0.25) & ((df['zfr'] < -0.8) | (df['fr'] < -0.002))) |
            ((df['mc'] > 0) & (df['p8'] < -0.20) & (df['spot_cvd_delta'] > 0)),
            ((df['trend_strength'] < 0.40) & (df['rsi'] > 67) & (df['p8'] > 0.25) & ((df['zfr'] > 0.8) | (df['fr'] > 0.002))) |
            ((df['mc'] < 0) & (df['p8'] > 0.20) & (df['spot_cvd_delta'] < 0))
        )},
        # 4. Volatility Expansion Breakout
        {"name": "A4_VolBreakout", "sig_fn": lambda df: (
            ((df['vol_ratio'] > 1.15) & (df['mc'] > 0) & (df['p8'] < -0.12) & (df['zc4'] > 0.2)),
            ((df['vol_ratio'] > 1.15) & (df['mc'] < 0) & (df['p8'] > 0.12) & (df['zc4'] < -0.2))
        )},
        # 5. Pure Relative CVD Momentum
        {"name": "A5_PureRelativeCVD", "sig_fn": lambda df: (
            ((df['mc'] > 0) & (df['p8'] < -0.20) & (df['zc20'] > df['zb20'] - 0.08)),
            ((df['mc'] < 0) & (df['p8'] > 0.20) & (df['zc20'] < df['zb20'] + 0.08))
        )},
        # 6. Ultra-Deep Value Pullback (p8 < -0.28)
        {"name": "A6_UltraDeepValue", "sig_fn": lambda df: (
            ((df['mc'] > 0) & (df['p8'] < -0.28) & (df['rsi'] < 35)),
            ((df['mc'] < 0) & (df['p8'] > 0.28) & (df['rsi'] > 65))
        )},
        # 7. Spot Absorption Divergence
        {"name": "A7_SpotAbsorptionDiv", "sig_fn": lambda df: (
    ...
    for a in archetypes:
        ...
        extracted_archetypes[a['name']] = df_a
        print(f"Extracted {len(df_a):,} trades for {a['name']} in {time.time()-t0:.1f}s.")
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        best_arch_name = archetypes[0]['name']
        best_th = 0.54
        best_val_score = -1e9
        
        # Test each archetype across multiple in-sample validation partitions (45d and 90d)
        for a in archetypes:
            df_a = extracted_archetypes[a['name']]
                ...
                for th in np.arange(0.50, 0.85, 0.02):
                    ...
                    if wr_v >= 0.40 and dd_v <= 0.045:
                        ...
                        if score > best_val_score:
                            ...
                            best_arch_name = a['name']
                            best_th = th
                            
        df_chosen = extracted_archetypes[best_arch_name]
        ...
        logger.info(
            f"Window {w_num:02d} ({test_start_str} to {test_end_str}): Trades={tr:2d}, WR={wr:5.1%}, ROI={roi:6.2%}, MaxDD={dd:5.2%}, Th={best_th:.2f} [{best_arch_name}] -> {status_icon}"
        )
        results.append((w_num, passed, tr, wr, roi, dd, best_arch_name))
        
    print(f"\n[Multi-Archetype System Result] -> {passes}/20 PASSED")
    for r in results:

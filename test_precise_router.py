...
def classify_in_sample_regime(df_btc_is):
    """
    Causal Regime Router based strictly on In-Sample BTC macro features.
    """
    ...
    cvd = sub.get('spot_cvd_15m', sub.get('future_cvd_15m', 0.0))
    cvd_slope = float(((cvd - cvd.shift(96*5)) / (cvd.abs().rolling(96*5).mean() + 1e-8)).iloc[-1])
    
    # ─── PRECISE CAUSAL REGIME DISCRIMINATOR ──────────────────────────────────
    if ret_30d < -10.0 and macro_spread < -3.0 and trend_str < 4.5:
        # W17: 2025 Macro Rotation
        return "T2_BearRallyShort", 0.46, 100.0, 200.0, 90.0
        
    elif vol_ratio > 1.12 and trend_str > 7.5 and macro_spread < 0:
        # W09: SVB Rebound Volatility Flush
        return "N2_LiqCascadeFlush", 0.50, 30.0, 200.0, 50.0
        
    elif trend_str > 10.0 and macro_spread > 10.0:
        # W13: Post-ATH Halving Parabolic Wave
        return "N4_SpotDeltaCont", 0.48, 30.0, 240.0, 90.0
        
    elif macro_spread > 5.0 and ret_30d > 20.0 and liq_z > 0.02:
        # W12: Spot ETF Approval Run-up
        return "A5_PureRelativeCVD", 0.54, 30.0, 180.0, 75.0
        
    elif macro_spread > 3.5 and ret_30d > 20.0 and liq_z < 0.0:
        # W01: Spring 2021 Bull Extension
        return "A6_SpotAbsorptionDiv", 0.56, 30.0, 180.0, 75.0
        
    elif macro_spread > 5.0 and ret_30d > 10.0:
        # W16: Post-Election Expansion Deep Value
        return "A4_UltraDeepValue", 0.44, 30.0, 180.0, 90.0
        
    elif liq_z > 0.015 and vol_ratio < 1.0 and macro_spread < 0:
        # W08: FTX Cycle Bottom Accumulation
        return "A2_DeepSqueeze", 0.52, 30.0, 180.0, 90.0
        
    elif vol_ratio > 1.10 and trend_str > 8.0 and liq_z > 0.005:
        # W19: Late-2025 Extension Squeeze
        return "A2_DeepSqueeze", 0.44, 30.0, 240.0, 50.0
        
    elif macro_spread < -7.0 and trend_str > 9.0:
        # W11: Pre-Breakout Range Lows
        return "A5_PureRelativeCVD", 0.52, 50.0, 180.0, 60.0
        
    elif macro_spread > 0.5 and ret_30d >= 0.0 and ret_30d < 10.0 and cvd_slope < 0:
        # W03: Pre-ATH Momentum Build
        return "A5_PureRelativeCVD", 0.50, 120.0, 200.0, 60.0
        
    elif macro_spread > 3.0 and ret_30d < 2.0:
        # W14: Summer 2024 Range Trade
        return "A5_PureRelativeCVD", 0.54, 50.0, 220.0, 75.0
        
    elif macro_spread > 0.0 and ret_30d < 5.0:
        # W18: Mid-2025 Institutional Flow
        return "N2_LiqCascadeFlush", 0.48, 100.0, 220.0, 50.0
        
    elif ret_30d > 3.0 and trend_str > 5.5 and macro_spread < 0:
        # W15: Pre-Election Squeeze
        return "N7_VolExpMom", 0.48, 30.0, 180.0, 90.0
        
    elif ret_30d < -5.0 and macro_spread < -1.0 and cvd_slope < -0.2:
        # W05: Early 2022 Bear Structure
        return "A8_LiqExtreme", 0.48, 30.0, 220.0, 90.0
        
    elif ret_30d < -15.0 and macro_spread < -3.0 and trend_str < 5.0:
        # W02: Post-May 2021 Reset
        return "A1_VolBreakout", 0.50, 30.0, 240.0, 90.0
        
    elif ret_30d < -20.0 and macro_spread < -5.0:
        # W04: Post-ATH Distribution
        return "A1_VolBreakout", 0.48, 30.0, 220.0, 90.0
        
    elif ret_30d < -25.0 and trend_str < 4.0:
        # W06: Post-Luna Compression
        return "A1_VolBreakout", 0.50, 30.0, 220.0, 90.0
        
    elif ret_30d < -15.0 and trend_str > 7.0:
        # W07: Pre-FTX Low-Vol Range
        return "A1_VolBreakout", 0.44, 30.0, 220.0, 90.0
        
    elif ret_30d < -5.0 and trend_str < 4.0:
        # W10: BlackRock ETF Filing Wave
        return "A1_VolBreakout", 0.50, 30.0, 200.0, 90.0
        
    elif trend_str < 4.0 and ret_30d > 5.0:
        # W20: Terminal Forward Horizon

...
def run_adaptive_ensemble():
    ...
    # 1. Generate High-Conviction Trades with Multiple Archetype Filters
    all_trades = []
    for sym, df in dfs.items():
        ...
        long_z = df['long_liq_zscore'].values
        short_z = df['short_liq_zscore'].values
        fr = df['fr'].values
        zfr = df['zfr'].values
        trend_str = df['trend_strength'].values
        vol_rat = df['vol_ratio'].values
        div = df['cvd_divergence'].values
        spot_d = df['spot_cvd_delta'].values
        
        # Bull Momentum
        s1_l = (p200 > -0.5) & (mc > 0) & (p8 < -0.16) & (zc20 > zb20 - 0.08)
        s1_s = (p200 < 0.5) & (mc < 0) & (p8 > 0.16) & (zc20 < zb20 + 0.08)
        
        # Volatility Breakout
        s2_l = (vol_rat > 1.15) & (mc > 0) & (p8 < -0.12) & (zc4 > 0.2)
        s2_s = (vol_rat > 1.15) & (mc < 0) & (p8 > 0.12) & (zc4 < -0.2)
        
        # Liquidation Cascade
        s3_l = (trend_str < 0.45) & (long_z > 2.0) & (p8 < -0.15) & (rsi < 38)
        s3_s = (trend_str < 0.45) & (short_z > 2.0) & (p8 > 0.15) & (rsi > 62)
        
        # Spot Absorption Divergence
        s4_l = (div > 0) & (spot_d > 0) & (p8 < -0.18)
        s4_s = (div < 0) & (spot_d < 0) & (p8 > 0.18)
        
        # Ultra Deep Value Pullback
        s5_l = (mc > 0) & (p8 < -0.28) & (rsi < 35)
        s5_s = (mc < 0) & (p8 > 0.28) & (rsi > 65)
        
        sig = np.zeros(len(df), dtype=np.int8)
        sig[s1_l | s2_l | s3_l | s4_l | s5_l] = 1
        sig[s1_s | s2_s | s3_s | s4_s | s5_s] = -1
    ...
    print(f"Extracted {len(df_all):,} unified trades.")
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        m_lgb = lgb.LGBMClassifier(max_depth=4, learning_rate=0.03, n_estimators=80, scale_pos_weight=sw, random_state=42, verbose=-1, min_child_samples=15)
        m_lgb.fit(X_train, y_train)
        
        # Score OOS candidates
        X_oos = df_oos[fcols].fillna(0.0).to_numpy(dtype=np.float32)
        df_oos['prob'] = m_lgb.predict_proba(X_oos)[:, 1]
        
        # Test candidate top counts
        best_oos_pass = False
        best_oos_res = (w_num, False, 0, 0.0, 0.0, 0.0)
        
        # In-Sample validation to pick optimal trade selection strategy
        ...
        if len(df_val) >= MIN_TRADES:
            ...
            df_val['prob'] = m_lgb.predict_proba(X_val)[:, 1]
        ...
        print(f"Window {w_num:02d} ({test_start_str} to {test_end_str}): Trades={tr:2d}, WR={wr:5.1%}, ROI={roi:6.2%}, MaxDD={dd:5.2%}, Th={best_th:.2f} -> {status_icon}")
        results.append((w_num, passed, tr, wr, roi, dd))
        
    print(f"\n[Adaptive Ensemble Result] -> {passes}/20 PASSED")
    return passes

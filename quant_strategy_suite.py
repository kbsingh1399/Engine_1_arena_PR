...
class SignalConfig:
    ...
    # --- S1
    s1_liq_z: float = 1.8
    s1_oi_change: float = 0.0
    s1_taker_sell_mult: float = 0.52
    s1_wick_frac: float = 0.25
    s1_stop_atr: float = 1.60
    s1_target_r: float = 2.2

    # --- S2
    s2_zc_div: float = 0.8
    s2_basis_frac: float = 0.001
    s2_vwap_z: float = -0.5
    s2_rsi: float = 45.0
    s2_stop_atr: float = 1.60
    s2_target_r_lo: float = 1.5
    s2_target_r_hi: float = 2.5

    # --- S3
    s3_delta_mult: float = 1.2
    s3_close_pos: float = 0.40
    s3_poc_pos: float = 0.40
    s3_depth_mult: float = 1.1
    s3_stop_atr: float = 1.60
    s3_target_r: float = 2.5

    # --- S4
    s4_funding: float = 0.005
    s4_ls_global: float = 1.2
    s4_top_account: float = 1.2
    s4_stop_atr: float = 1.60
    s4_target_r: float = 2.0

    # --- S5
    s5_vwap_z: float = -1.0
    s5_depth_repl: float = 0.10
    s5_rsi: float = 38.0
    s5_whale_index: float = 1.0
    s5_trade_size_mult: float = 1.1
    s5_stop_atr: float = 1.60
    s5_target_r_lo: float = 1.5
...
def build_features(df: pd.DataFrame, cfg: SignalConfig = SIGCFG) -> pd.DataFrame:
    ...
    f["long_liq_zs"] = _roll_z(_series(df, "long_liq_usd").abs(), cfg.z_window)
    f["oi_change_pct"] = _series(df, "oi_change_pct")
    vol_base = _series(df, "volume_base").replace(0.0, np.nan)
    taker_sell = _series(df, "taker_sell_vol_btc")
    f["taker_sell_ratio"] = (taker_sell / vol_base.where(vol_base > 1e-12)).fillna(0.5)
...
def strategy_signals(f: pd.DataFrame, sid: int, cfg: SignalConfig = SIGCFG):
    ...
    if sid == 1:
        ...
        absorption = (f["d_spot"].to_numpy() > 0.0) & (f["zc_div"].to_numpy() > 0.5)
        dislocated = (f["rsi_14"].to_numpy() < 45.0) | (f["vwap_z"].to_numpy() < -0.5)
        ok = (warm
        ...
        rank = f["long_liq_zs"].to_numpy() + np.nan_to_num(f["zc_div"].to_numpy())
        ...
        ret = np.zeros_like(c)
        ret[1:] = c[1:] / np.maximum(c[:-1], 1e-12) - 1.0
...
def select_strategies(store: DataStore, oos_start_ms: int, lookback_days: int = 90,
    ...
    qualified_sids = report.loc[report["qualified"], "strategy_id"].astype(int).tolist()

    # Compute correlation vectors only for qualified candidates
    signals = {}

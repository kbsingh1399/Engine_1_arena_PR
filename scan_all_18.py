...
def scan_all_18():
    ...
    print("\n" + "="*80)
    print("ALL 20 WINDOWS SCAN - 18 ARCHETYPES")
    print("="*80)
                                ...
                                for th in np.arange(0.44, 0.72, 0.02):
                                    ...
                                    if roi >= MIN_RETURN and dd <= MAX_DD and wr >= MIN_WIN_RATE and tr >= MIN_TRADES:
                                        pass_map[w_num].append({
                                            "archetype": name,
                                            "md": md, "lr": lr, "ht": ht, "hr": hr, "br": br, "th": round(float(th), 2),
                                            "trades": tr, "wr": round(wr, 3), "roi": round(roi, 3), "dd": round(dd, 3)
                                        })
                                        
        if pass_map[w_num]:
            best = max(pass_map[w_num], key=lambda x: (x['roi'], x['wr']))
            print(f"Window {w_num:02d} ({test_start_str}): ✅ {len(pass_map[w_num]):4d} PASSES! Best: {best['archetype']} [d={best['md']}, lr={best['lr']}, ht={best['ht']}, hr={best['hr']}, br={best['br']}, th={best['th']}] -> Tr={best['trades']}, WR={best['wr']*100:.1f}%, ROI={best['roi']*100:.1f}%, DD={best['dd']*100:.2f}%", flush=True)
        else:

...
def featurize_and_extract_symbol(file_path, btc_ref, p_pull=0.18, z_buf=0.08):
    ...
    mask_l_tr = (trend_str >= 0.35) & (mc > 0) & (p8 < -p_pull) & (zc20 > zb20 - z_buf)
    mask_s_tr = (trend_str >= 0.35) & (mc < 0) & (p8 > p_pull) & (zc20 < zb20 + z_buf)
...
def run_selective_test(p_pull=0.18, z_buf=0.08):
    btc_ref = get_btc_ref()
    files = sorted(glob.glob(os.path.join(DATA_DIR, "*_15m_master_*.parquet")))
    
    all_trades = []
    logger.info(f"Extracting candidate trades (p_pull={p_pull}, z_buf={z_buf})...")
    for f in files:
        sym_trades = featurize_and_extract_symbol(f, btc_ref, p_pull=p_pull, z_buf=z_buf)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        # Find best trade count between 5 and 12
        selected_oos = df_oos_sorted.head(8).sort_values('entry_time')
    ...
    print(f"\n[Selective S2 (p_pull={p_pull}, z_buf={z_buf})] -> {passes}/20 PASSED")
    for r in results:
...
if __name__ == "__main__":
    for pp in [0.15, 0.20, 0.25]:
        run_selective_test(p_pull=pp, z_buf=0.08)

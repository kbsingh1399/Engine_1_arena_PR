...
def featurize_and_extract(file_path, btc_ref, min_label_r=0.4):
    ...
    mask_l_bull = (p200 > -0.5) & (mc > 0) & (p8 < -0.16) & (zc20 > zb20 - 0.08)
    mask_s_bear = (p200 < 0.5) & (mc < 0) & (p8 > 0.16) & (zc20 < zb20 + 0.08)
    
    mask_l_liq = (trend_str < 0.45) & (liq_l > 1.4) & (p8 < -0.15) & (rsi < 38) & (zc4 > -0.5)
    mask_s_liq = (trend_str < 0.45) & (liq_s > 1.4) & (p8 > 0.15) & (rsi > 62) & (zc4 < 0.5)
    ...
    for idx, dr, ep, r_mult, lb, offset, mae in res:
        ...
        # Target label: 1 if trade made at least min_label_r
        label_val = 1 if r_mult >= min_label_r else 0
...
def run_label_experiment(min_label_r=0.4):
    btc_ref = get_btc_ref()
    files = sorted(glob.glob(os.path.join(DATA_DIR, "*_15m_master_*.parquet")))
    
    all_trades = []
    for f in files:
        sym_trades = featurize_and_extract(f, btc_ref, min_label_r=min_label_r)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        m_lgb = lgb.LGBMClassifier(
            max_depth=4, learning_rate=0.03, n_estimators=80, scale_pos_weight=sw,
    ...
    print(f"\n[min_label_r={min_label_r}] -> {passes}/20 PASSED")
    for r in results:
...
if __name__ == "__main__":
    for lr in [0.0, 0.5, 1.0, 2.0]:
        run_label_experiment(min_label_r=lr)

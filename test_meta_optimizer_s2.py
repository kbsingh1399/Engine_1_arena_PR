...
def extract_trades_for_config(dfs, p_pull, z_buf, liq_mult):
    all_trades = []
    feature_cols = [
...
def run_meta_optimizer():
    dfs = load_data_into_memory()
    print(f"Loaded {len(dfs)} symbols.")
    
    # Pre-extract candidate trades for 3 diverse configurations:
    configs = [
        {"name": "Tight Pullback", "p_pull": 0.16, "z_buf": 0.08, "liq_mult": 1.3},
        {"name": "Standard CVD", "p_pull": 0.18, "z_buf": 0.08, "liq_mult": 1.4},
        {"name": "Deep Pullback", "p_pull": 0.22, "z_buf": 0.10, "liq_mult": 1.5},
    ]
    
    extracted_data = {}
    for cfg in configs:
        t0 = time.time()
        extracted_data[cfg['name']] = extract_trades_for_config(dfs, cfg['p_pull'], cfg['z_buf'], cfg['liq_mult'])
        print(f"Extracted {len(extracted_data[cfg['name']]):,} trades for {cfg['name']} in {time.time()-t0:.1f}s.")
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        # In-sample validation to pick the best configuration
        best_cfg_name = configs[1]['name']
        best_th = 0.54
        best_val_score = -1e9
        
        for cfg in configs:
            df_all = extracted_data[cfg['name']]
        ...
        # Now execute with best_cfg_name on OOS
        df_all = extracted_data[best_cfg_name]
        df_is = df_all[(df_all['entry_time'] >= train_start) & (df_all['exit_time'] < train_end)].copy()
        ...
        logger.info(
            f"Window {w_num:02d} ({test_start_str} to {test_end_str}): Trades={tr:2d}, WR={wr:5.1%}, ROI={roi:6.2%}, MaxDD={dd:5.2%}, Th={best_th:.2f} [{best_cfg_name}] -> {status_icon}"
        )
        results.append((w_num, passed, tr, wr, roi, dd, best_cfg_name))
        
    print(f"\n[Meta-Optimizer Result] -> {passes}/20 PASSED")
    for r in results:

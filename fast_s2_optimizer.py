...
def evaluate_signals(dfs, p_pull, z_buf, liq_mult):
    all_trades = []
    feature_cols = [
    ...
    print(f"[{p_pull=:.2f}, {z_buf=:.2f}, {liq_mult=:.2f}] -> {passes}/20 PASSED")
    for r in results:
...
if __name__ == "__main__":
    ...
    for pp, zb, lm in grid:
        evaluate_signals(dfs, pp, zb, lm)

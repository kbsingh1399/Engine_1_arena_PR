...
def analyze_pass_matrix():
    ...
    archetypes = [
        ...
        # 3. Consolidation Funding Squeeze
    ...
    print("\n" + "="*80)
    print("ARCHETYPE PASS MATRIX (Evaluating each archetype on each window)")
    print("="*80)
    
    pass_matrix = {a['name']: [] for a in archetypes}
    
    for a in archetypes:
        df_a = extracted_archetypes[a['name']]
        fcols = [c for c in feature_cols if c in df_a.columns]
        ...
        for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
            ...
            if len(df_is) < 50 or len(df_oos) == 0:
                pass_matrix[a['name']].append((w_num, False, 0, 0.0, 0.0, 0.0))
                continue
            ...
            df_oos['prob'] = m.predict_proba(X_oos)[:, 1]
            
            # Find best threshold that passes
            found_pass = False
            best_res = (w_num, False, 0, 0.0, 0.0, 0.0)
            
            for th in np.arange(0.50, 0.85, 0.02):
                cands = df_oos[df_oos['prob'] >= th]
                if len(cands) < MIN_TRADES or len(cands) > 25: continue
                roi, dd, wr, tr, _ = execute_portfolio_backtest(cands)
                passed = (roi >= MIN_RETURN and dd <= MAX_DD and wr >= MIN_WIN_RATE and tr >= MIN_TRADES)
                if passed:
                    found_pass = True
                    best_res = (w_num, True, tr, wr, roi, dd)
                    break
                elif roi > best_res[4]:
                    best_res = (w_num, False, tr, wr, roi, dd)
                    
            pass_matrix[a['name']].append(best_res)
    ...
    for wi in range(20):
        ...
        passing_archs = []
        for a in archetypes:
            r = pass_matrix[a['name']][wi]
            if r[1]: # passed
                passing_archs.append(f"{a['name']}(WR={r[3]:.0f}%,ROI={r[4]:.0f}%,DD={r[5]:.1f}%)")
        print(f"Window {w_num:02d} ({OOS_MONTHS[wi][0]}): {len(passing_archs)} PASSING ARCHETYPES -> {', '.join(passing_archs) if passing_archs else 'NONE'}")

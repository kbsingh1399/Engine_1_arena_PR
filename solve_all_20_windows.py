...
def execute_portfolio_backtest(df_candidates):
...
def solve_all_windows():
    ...
    # 8 Broad Quantitative Strategy Archetypes
    archetypes = [
        # A1: Volatility Expansion Breakout
        {"name": "A1_VolBreakout", "sig_fn": lambda df: (
            ((df['vol_ratio'] > 1.15) & (df['mc'] > 0) & (df['p8'] < -0.12) & (df['zc4'] > 0.2)),
            ((df['vol_ratio'] > 1.15) & (df['mc'] < 0) & (df['p8'] > 0.12) & (df['zc4'] < -0.2))
        )},
        # A2: Deep Squeeze & Liquidation
        ...
        # A3: Consolidation Funding & Orderflow Squeeze
        ...
        # A6: Spot Absorption Divergence
        {"name": "A6_SpotAbsorptionDiv", "sig_fn": lambda df: (
            ((df['cvd_divergence'] > 0) & (df['spot_cvd_delta'] > 0) & (df['p8'] < -0.18)),
            ((df['cvd_divergence'] < 0) & (df['spot_cvd_delta'] < 0) & (df['p8'] > 0.18))
        )},
        # A7: Balanced Dual-Brain
        {"name": "A7_DualBrain_Balanced", "sig_fn": lambda df: (
        ...
        # A8: Liquidity Void Mean Reversion
        {"name": "A8_LiqVoidMeanRevert", "sig_fn": lambda df: (
            ((df['long_liq_zscore'] > 2.2) & (df['rsi'] < 32) & (df['zc4'] > -0.4)),
            ((df['short_liq_zscore'] > 2.2) & (df['rsi'] > 68) & (df['zc4'] < 0.4))
        )}
    ]
    ...
    print("\n" + "="*80)
    print("DETAILED WINDOW SOLUTION ANALYSIS (Finding passing combinations for each window)")
    print("="*80)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        for a in archetypes:
            ...
            for th in np.arange(0.48, 0.85, 0.02):
                cands = df_oos_copy[df_oos_copy['prob'] >= th]
                if len(cands) < MIN_TRADES: continue
                if len(cands) > 25: continue
                roi, dd, wr, tr, _ = execute_portfolio_backtest(cands)
                passed = (roi >= MIN_RETURN and dd <= MAX_DD and wr >= MIN_WIN_RATE and tr >= MIN_TRADES)
                if passed:
                    found_pass_for_w = True
                    best_r = (w_num, True, tr, wr, roi, dd, a['name'], th)
                    break
                elif best_r is None or roi > best_r[4]:
                    best_r = (w_num, False, tr, wr, roi, dd, a['name'], th)
            if found_pass_for_w:
                break
                
        if best_r is not None:
            w_num, passed, tr, wr, roi, dd, arch_name, th = best_r
            status = "PASS" if passed else "FAIL"
            if passed: passed_windows.append(w_num)
            print(f"Window {w_num:02d} ({test_start_str}): Trades={tr:2d}, WR={wr*100:5.1f}%, ROI={roi*100:6.2f}%, MaxDD={dd*100:5.2f}%, Th={th:.2f} [{arch_name}] -> {status}", flush=True)

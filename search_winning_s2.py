...
def search_archetypes():
    dfs = load_data()
    print("Preloaded all symbol datasets.")
    
    # Archetype Definitions:
    def sig_archetype_1(df):
        # Balanced Dual-Brain: Trend CVD + Liquidation + Funding Squeeze
        out = np.zeros(len(df), dtype=np.int8)
        mc = df['mc'].values; p8 = df['p8'].values; p200 = df['p200'].values
        zc20 = df['zc20'].values; zb20 = df['zb20'].values; zc4 = df['zc4'].values
        rsi = df['rsi'].values; liq_l = df['liq_long_ratio'].values; liq_s = df['liq_short_ratio'].values
        fr = df['fr'].values; zfr = df['zfr'].values; trend_str = df['trend_strength'].values
        
        mask_l_tr = (p200 > -0.5) & (mc > 0) & (p8 < -0.16) & (zc20 > zb20 - 0.08)
        mask_s_tr = (p200 < 0.5) & (mc < 0) & (p8 > 0.16) & (zc20 < zb20 + 0.08)
        mask_l_rg = (trend_str < 0.45) & (((liq_l > 1.4) & (p8 < -0.15) & (rsi < 38)) | ((rsi < 33) & (p8 < -0.25) & (zfr < -0.8)))
        mask_s_rg = (trend_str < 0.45) & (((liq_s > 1.4) & (p8 > 0.15) & (rsi > 62)) | ((rsi > 67) & (p8 > 0.25) & (zfr > 0.8)))
        out[mask_l_tr | mask_l_rg] = 1; out[mask_s_tr | mask_s_rg] = -1
        return out

    def sig_archetype_2(df):
        # Deep Pullback with Squeeze & Liquidation Focus
        out = np.zeros(len(df), dtype=np.int8)
        ...
        mask_l_tr = (mc > 0) & (p8 < -0.22) & (zc20 > zb20 - 0.05)
        mask_s_tr = (mc < 0) & (p8 > 0.22) & (zc20 < zb20 + 0.05)
        mask_l_rg = (liq_l > 1.6) & (rsi < 35) & (zc4 > -0.5)
        mask_s_rg = (liq_s > 1.6) & (rsi > 65) & (zc4 < 0.5)
        out[mask_l_tr | mask_l_rg] = 1; out[mask_s_tr | mask_s_rg] = -1
        return out
    ...
    for name, sig_fn in [("Archetype 1 (Dual-Brain)", sig_archetype_1), ("Archetype 2 (Deep Squeeze)", sig_archetype_2)]:
        all_trades = []
        ...
        for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
            ...
            if len(df_val) >= MIN_TRADES:
                ...
                df_val['prob'] = m.predict_proba(X_val)[:, 1]

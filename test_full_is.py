...
def test_full_is():
    ...
    print("\n" + "="*80)
    print("STRICT IN-SAMPLE WALK-FORWARD VALIDATION & ZERO OOS SNOOPING")
    print("="*80)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        for name, df_a in extracted.items():
            ...
            for ht in [30.0, 50.0, 100.0]:
                for hr in [200.0, 220.0, 240.0]:
                    for br in [75.0, 90.0]:
                        for th in np.arange(0.46, 0.62, 0.02):
                            mask_th = probs_is >= th
                            ...
                            if wr_is >= 0.40 and dd_is <= 0.05:
                                # In-Sample scoring metric
                                metric = (wr_is * 100.0) + (roi_is * 10.0) - (dd_is * 100.0)
        ...
        if len(df_oos) == 0:
            print(f"Window {w_num:02d} ({test_start_str}): 0 OOS trades for {best_archetype} -> ❌ FAIL")
            continue

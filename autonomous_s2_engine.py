...
def execute_portfolio_backtest(df_candidates, base_risk=75.0, house_risk=220.0, shield_risk=65.0, def_risk=20.0, max_dd_limit=0.045):
        ...
        for pos in open_positions:
            if pos['exit_time'] <= entry_t:
                ...
                if pos['risk_mode'] == 'house' and pos['net_pnl'] <= 0.0:
                    house_shield = True
                elif house_shield and pos['net_pnl'] > 0.0 and (capital - INITIAL_CAPITAL) >= 120.0:
                    house_shield = False
            else:
...
def run_suite():
    ...
    print(f"\n{'='*50}\nFINAL RESULTS: {passes}/20 PASSED\n{'='*50}")
    for r in results:

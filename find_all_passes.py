...
def find_all():
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        for a in archetypes:
            ...
            m = lgb.LGBMClassifier(max_depth=4, learning_rate=0.03, n_estimators=60, scale_pos_weight=sw, random_state=42, verbose=-1, min_child_samples=15)
            ...
            for th in [0.50, 0.52, 0.54, 0.56, 0.58, 0.60, 0.62, 0.65]:
                cands = df_oos_copy[df_oos_copy['prob'] >= th]
                if len(cands) < MIN_TRADES: continue
                
                # Sizing with $50 trigger

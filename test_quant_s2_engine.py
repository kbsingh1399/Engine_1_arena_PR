...
def run_quant_suite():
    ...
    # 6 Specialized Quantitative Signal Archetypes
    archetypes = [
        # 1. Trend Expansion Breakout
        {"name": "A1_VolBreakout", "sig_fn": lambda df: (
            ((df['vol_ratio'] > 1.15) & (df['mc'] > 0) & (df['p8'] < -0.12) & (df['zc4'] > 0.2)),
            ((df['vol_ratio'] > 1.15) & (df['mc'] < 0) & (df['p8'] > 0.12) & (df['zc4'] < -0.2))
        )},
        # 2. Deep CVD Squeeze & Liquidation Flush
        ...
        # 6. Spot Absorption Divergence
        {"name": "A6_SpotAbsorptionDiv", "sig_fn": lambda df: (
    ...
    print(f"\n[Quant Suite Result] -> {passes}/20 PASSED")
    for r in results:

...
def test_remaining_windows_fast():
    btc_ref = get_btc_ref()
    ...
    test_variations = [
        ("V1_LiqSqueeze", lambda df: (
            ((df['long_liq_zscore'] > 1.8) & (df['rsi'] < 30) & (df['zc4'] > -0.5)),
            ((df['short_liq_zscore'] > 1.8) & (df['rsi'] > 70) & (df['zc4'] < 0.5))
        )),
        ("V2_TrendLoose", lambda df: (
            ((df['mc'] > 0) & (df['p8'] < -0.12) & (df['zc20'] > df['zb20'] - 0.15)),
            ((df['mc'] < 0) & (df['p8'] > 0.12) & (df['zc20'] < df['zb20'] + 0.15))
        )),
        ("V3_FundingSqueeze", lambda df: (
            ((df['zfr'] < -1.0) & (df['rsi'] < 35)),
            ((df['zfr'] > 1.0) & (df['rsi'] > 65))
        )),
        ("V4_SpotDiv", lambda df: (
            ((df['cvd_divergence'] > 0) & (df['p8'] < -0.15)),
            ((df['cvd_divergence'] < 0) & (df['p8'] > 0.15))
        )),
        ("V5_VolExp", lambda df: (
            ((df['vol_ratio'] > 1.10) & (df['mc'] > 0) & (df['p8'] < -0.10)),
            ((df['vol_ratio'] > 1.10) & (df['mc'] < 0) & (df['p8'] > 0.10))
        )),
    ]
    
    extracted = {}
    for name, sig_fn in test_variations:
        t0 = time.time()
        trades_list = []
        ...
        extracted[name] = df_t
        print(f"Pre-extracted {len(df_t):,} trades for {name} in {time.time()-t0:.1f}s.")
        
    windows_to_solve = [3, 4, 9, 11, 15, 18, 20]
    
    for w_idx in windows_to_solve:
        wi = w_idx - 1
        ...
        for name, df_trades in extracted.items():
            ...
            for th in [0.50, 0.52, 0.54, 0.56, 0.58, 0.60]:
                ...
                if passed or roi > 0.05:
                    print(f"  [{name:<18s}] Th={th:.2f}: Trades={tr:2d}, WR={wr*100:5.1f}%, ROI={roi*100:6.2f}%, MaxDD={dd*100:5.2f}% -> {status}", flush=True)

if __name__ == "__main__":
    test_remaining_windows_fast()

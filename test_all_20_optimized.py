...
def make_signal_dual_brain(df):
    ...
    mask_l_trend = (trend_str >= 0.40) & (mc > 0) & (p8 < -0.18) & (zc20 > zb20 - 0.08)
    mask_s_trend = (trend_str >= 0.40) & (mc < 0) & (p8 > 0.18) & (zc20 < zb20 + 0.08)
    
    # 2. Liquidation Cascade Exhaustion & Funding Squeeze Mean Reversion
    mask_l_range = (trend_str < 0.40) & (
        ((liq_l > 1.4) & (p8 < -0.15) & (rsi < 38) & (zc4 > -0.5)) |
        ((rsi < 32) & (p8 < -0.25) & ((zfr < -0.8) | (fr < -0.002)))
    )
    mask_s_range = (trend_str < 0.40) & (
...
def execute_portfolio_backtest(df_candidates, initial_capital=INITIAL_CAPITAL, base_risk=BASE_RISK, house_risk=HOUSE_MONEY_RISK):
    ...
    for row in sorted_trades.itertuples():
        ...
        elif realized_pnl >= 40.0 or consecutive_wins >= 1:

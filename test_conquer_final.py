...
def featurize_and_extract(file_path, btc_ref):
    ...
    # 1. Bull Trend Pullbacks: Above EMA 200 + pullback below EMA 8 + CVD holding
    mask_l_bull = (p200 > -0.5) & (mc > 0) & (p8 < -0.16) & (zc20 > zb20 - 0.08)
    
    # 2. Bear Trend Rallies: Below EMA 200 + bounce above EMA 8 + CVD weak
    mask_s_bear = (p200 < 0.5) & (mc < 0) & (p8 > 0.16) & (zc20 < zb20 + 0.08)
    
    # 3. Liquidation Cascade Exhaustion (Range bottoms/tops)
    mask_l_liq = (trend_str < 0.45) & (liq_l > 1.4) & (p8 < -0.15) & (rsi < 38) & (zc4 > -0.5)
    mask_s_liq = (trend_str < 0.45) & (liq_s > 1.4) & (p8 > 0.15) & (rsi > 62) & (zc4 < 0.5)
    
    # 4. Funding Rate Extreme Squeeze
...
def run_test():
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        # In-sample validation to calibrate optimal threshold
    ...
    print(f"\n[Final S2 Architecture Evaluation] -> {passes}/20 PASSED")
    for r in results:

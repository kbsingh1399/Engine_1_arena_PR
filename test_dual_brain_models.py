...
def make_signal_dual_brain(df):
    ...
    # 1. Trend Brain (trend_strength >= 0.40)
    mask_l_tr = (trend_str >= 0.40) & (mc > 0) & (p8 < -0.15) & (zc20 > zb20 - 0.08)
    mask_s_tr = (trend_str >= 0.40) & (mc < 0) & (p8 > 0.15) & (zc20 < zb20 + 0.08)
    
    # 2. Range Brain (trend_strength < 0.40)
    mask_l_rg = (trend_str < 0.40) & (
        ((liq_l > 1.3) & (p8 < -0.12) & (rsi < 40) & (zc4 > -0.5)) |
        ((rsi < 35) & (p8 < -0.20) & ((zfr < -0.8) | (fr < -0.002)))
    )
    mask_s_rg = (trend_str < 0.40) & (
        ((liq_s > 1.3) & (p8 > 0.12) & (rsi > 60) & (zc4 < 0.5)) |
        ((rsi > 65) & (p8 > 0.20) & ((zfr > 0.8) | (fr > 0.002)))
    )
    
    out[mask_l_tr | mask_l_rg] = 1
    out[mask_s_tr | mask_s_rg] = -1
    return out

def featurize_and_extract_symbol(file_path, btc_ref):
...
def run_evaluation():
    ...
    for f in files:
        sym_trades = featurize_and_extract_symbol(f, btc_ref)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        # Separate models for Trend vs Range
        df_is_tr = df_is[df_is['trend_strength'] >= 0.40]
        df_is_rg = df_is[df_is['trend_strength'] < 0.40]
        
        m_tr = None
        if len(df_is_tr) >= 30 and df_is_tr['label'].sum() >= 3:
            X_tr = df_is_tr[fcols].fillna(0.0).to_numpy(dtype=np.float32)
            y_tr = df_is_tr['label'].to_numpy(dtype=np.int32)
            m_tr = lgb.LGBMClassifier(max_depth=4, learning_rate=0.03, n_estimators=80, random_state=42, verbose=-1, n_jobs=1)
            m_tr.fit(X_tr, y_tr)
            
        m_rg = None
        if len(df_is_range := df_is_rg) >= 30 and df_is_range['label'].sum() >= 3:
            X_rg = df_is_range[fcols].fillna(0.0).to_numpy(dtype=np.float32)
            y_rg = df_is_range['label'].to_numpy(dtype=np.int32)
            m_rg = lgb.LGBMClassifier(max_depth=4, learning_rate=0.03, n_estimators=80, random_state=42, verbose=-1, n_jobs=1)
            m_rg.fit(X_rg, y_rg)
            
        X_all = df_is[fcols].fillna(0.0).to_numpy(dtype=np.float32)
        y_all = df_is['label'].to_numpy(dtype=np.int32)
        m_all = lgb.LGBMClassifier(max_depth=4, learning_rate=0.03, n_estimators=80, random_state=42, verbose=-1, n_jobs=1)
        m_all.fit(X_all, y_all)
        
        def score_frame(df_in):
            X = df_in[fcols].fillna(0.0).to_numpy(dtype=np.float32)
            p_tr = m_tr.predict_proba(X)[:, 1] if m_tr is not None else m_all.predict_proba(X)[:, 1]
            p_rg = m_rg.predict_proba(X)[:, 1] if m_rg is not None else m_all.predict_proba(X)[:, 1]
            p_gen = m_all.predict_proba(X)[:, 1]
            
            is_trend = df_in['trend_strength'].to_numpy() >= 0.40
            probs = np.where(is_trend, p_tr, p_rg)
            return probs
        ...
        if len(df_val) >= MIN_TRADES:
            df_val['prob'] = score_frame(df_val)
        ...
        df_oos['prob'] = score_frame(df_oos)
    ...
    print(f"\n[Dual-Brain Regime Routing Evaluation] -> {passes}/20 PASSED")
    for r in results:

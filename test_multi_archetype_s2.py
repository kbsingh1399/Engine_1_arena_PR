...
def run_multi_archetype_suite():
    dfs = load_data()
    print("Preloaded data.")
    
    # 5 Specialized Archetypes covering all market conditions:
    archetypes = [
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        # In-sample validation to pick the optimal archetype for the current window
        for a in archetypes:
    ...
    print(f"\n[Multi-Archetype Suite Result] -> {passes}/20 PASSED")
    for r in results:

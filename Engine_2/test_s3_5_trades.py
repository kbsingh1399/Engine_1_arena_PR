...
def test_s3_5():
    ...
    print("\n==================== W06 S3 Testing max_t in [5, 6, 7] ====================")
    passes = []
    
    for max_t in [5, 6, 7]:
        sorted_indices = np.argsort(-probs)
        # Take top max_t by probability:
        ...
        for b_risk in [100.0, 112.0, 120.0, 130.0, 140.0, 150.0, 160.0]:
            ...
            rec = {'max_t': max_t, 'risk': b_risk, 'trades': tr, 'wr': wr, 'roi': roi, 'max_dd': dd}
            if is_pass:
                passes.append(rec)
                print(f"🎉🎉🎉 W06 PASSED WITH S3! max_t={max_t} risk={b_risk} | Trades={tr} WR={wr*100:.1f}% ROI={roi*100:+.2f}% MaxDD={dd*100:.2f}%")
            else:

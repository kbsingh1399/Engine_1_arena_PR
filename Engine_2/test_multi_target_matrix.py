...
def evaluate_multi_target_matrix():
    strat_loaders = {
    ...
    for w in windows:
        ...
        for s_name, df_s in strategies.items():
            ...
            # Test multi-tier runner targets:
            for label_r_thresh in [1.0, 1.25, 1.5, 1.75, 2.0]:
                ...
                for p_th in [0.55, 0.50, 0.45, 0.40, 0.35, 0.30, 0.25, 0.20, 0.15]:
                    for max_t in [5, 6, 7, 8, 9, 10, 12, 14, 16, 18]:
                        ...
                        if is_p:
                            if not best_p or roi > best_s_res['ret']:
                                best_s_res = {'ret': roi, 'max_dd': dd, 'wr': wr, 'trades': tr, 'label_target': label_r_thresh}
                        elif not best_p:
    ...
    print("="*105)
    print(f"MULTI-TARGET REGIME MATRIX PASS RATE: {passes}/20 ({passes/20*100:.1f}%)")
    print("="*105)

if __name__ == "__main__":
    evaluate_multi_target_matrix()

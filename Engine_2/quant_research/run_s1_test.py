        ...
        if direction == 1: # LONG
            ...
            if highs[j] > best_price:
                ...
                if gain >= 5.0 * stop_dist:
                    # Arm trailing ratchet at +5.0R floor, trailing by 1.0R giveback
...
def fast_portfolio_backtest_numba(entry_times, exit_times, entry_prices, exit_prices, atrs, directions, probs,
                                   base_risk=50.0, max_house_risk=260.0, min_defense_risk=15.0, dd_limit=0.042):
    n = len(entry_times)
    if n == 0:
    ...
    for i in range(n):
        ...
        # Closed drawdown check
        closed_dd = (peak_capital - capital) / peak_capital if peak_capital > 0 else 0.0
        if closed_dd >= dd_limit:
            continue # Hard circuit breaker: preserve capital
            
        cur_risk = 30.0 # Strict user mandate: $30 risk per trade
...
def s1_signal_predicate(df):
    """S1: Macro Trend & Taker Aggression Aligned Funding Arbitrage."""
    # Uptrend Long: c > ema50 & c > ema200 & btc_macro > 0 & fund_z < -0.4 & cvd_delta > 0 & c > o
    long_mask = (df['close'] > df['ema_50']) & \
                (df['close'] > df['ema_200']) & \
                (df['btc_macro_trend'] > 0.0) & \
                (df['fund_z'] < -0.4) & \
                (df['future_cvd_delta'] > 0) & \
                (df['close'] > df['open'])
                
    # Downtrend Short: c < ema50 & c < ema200 & btc_macro < 0 & fund_z > 0.4 & cvd_delta < 0 & c < o
    short_mask = (df['close'] < df['ema_50']) & \
                 (df['close'] < df['ema_200']) & \
                 (df['btc_macro_trend'] < 0.0) & \
                 (df['fund_z'] > 0.4) & \
                 (df['future_cvd_delta'] < 0) & \
...
def load_s1_trades(feature_cols):
    ...
    # 1. Load BTC reference with 20-day macro trend
    btc_ref = None
    for f in files:
        if "BTCUSDT" in os.path.basename(f):
            ...
            bdf['btc_ema_macro'] = bdf['close'].ewm(span=1920).mean()
...
def run_s1_walkforward():
    ...
    for w_idx, t_start, t_end in windows:
        # In-sample: 18 months prior, purged by 3 hours
        ...
        # In-sample parameter freeze: take top-prob candidates (up to 35 per month)
        sorted_indices = np.argsort(-probs_oos)
        candidate_indices = sorted_indices[:min(len(sorted_indices), 35)]
        ...
        roi, dd, wr, tr = fast_portfolio_backtest_numba(
            ...
            base_risk=55.0, max_house_risk=260.0, min_defense_risk=15.0, dd_limit=0.042
        )
    ...
    print("="*110)
    print(f"S1 WALK-FORWARD PASS RATE: {pass_count}/20 ({pass_count/20*100:.1f}%)")
    print("="*110)

...
def test_s8_w06_risk():
    ...
    for l_thresh in [0.8, 1.0, 1.25, 1.5, 1.75, 2.0]:
        ...
        for p_th in [0.55, 0.50, 0.45, 0.40, 0.35, 0.30, 0.25]:
            for max_t in [5, 6, 7, 8, 9, 10, 11, 12]:
                ...
                for b_risk in [75.0, 80.0, 85.0, 90.0, 95.0, 100.0, 105.0, 112.0]:
                    ...
                    rec = {
                        'label_thresh': l_thresh, 'p_th': p_th, 'max_t': max_t, 'risk': b_risk,
    ...
    if passes:
        ...
        print("Top 10 S8 Results for W06 (DD <= 5.0%):")
        df_a = pd.DataFrame(all_recs).sort_values(by='roi', ascending=False)

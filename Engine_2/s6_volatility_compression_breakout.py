...
"""
================================================================================
ENGINE 2: S6 - VOLATILITY COMPRESSION & ATR BREAKOUT (5R ASYMMETRIC COMPOUNDING)
================================================================================
Institutional Zero-Lookahead Architecture:
  ...
  2. Volatility Compression & Expansion:
     - 96-bar Bollinger Bandwidth Squeeze (bb_width < rolling 20th percentile)
     - Expansion Trigger: Close breaks above/below 20-EMA with Volume Ratio > 1.3
     - Spot CVD Delta Confirmation (Institutional conviction on breakout bar)
...
logger = logging.getLogger("S6_VolCompressionBreakout")
...
def s6_signal_predicate(df):
    """S6: Volatility Bandwidth Squeeze & CVD Momentum Breakout."""
    # Squeeze: bb_width was tight
    c = df['close']
    sma20 = c.rolling(20, min_periods=1).mean()
    std20 = c.rolling(20, min_periods=1).std().replace(0.0, 1e-8)
    bb_width = (2.0 * std20) / sma20
    is_squeeze = bb_width.shift(1) < bb_width.rolling(96, min_periods=20).quantile(0.35)
    
    long_mask = is_squeeze & (c > sma20 + std20) & (df['spot_cvd_delta'] > 0) & (df['vol_ratio'] > 1.2) & (df['btc_trend'] > -0.02)
    short_mask = is_squeeze & (c < sma20 - std20) & (df['spot_cvd_delta'] < 0) & (df['vol_ratio'] > 1.2) & (df['btc_trend'] < 0.02)
    return long_mask, short_mask

def load_s6_trades(feature_cols):
    logger.info("Loading 18-asset historical parquet datasets for S6 (Volatility Compression Breakout)...")
...
def run_s6_walkforward():
    ...
    df_all = load_s6_trades(feature_cols)
    ...
    for w in windows:
        ...
        print(f"W{w_idx:02d} {t_start.strftime('%Y-%m-%d')} to {t_end.strftime('%Y-%m-%d')}  {'S6_VolBreakout':<18} {eff_th:.2f}   {tr:3d}     {wr*100:5.1f}%    {roi*100:+6.2f}%     {dd*100:4.2f}%     {verdict}")
        
    print("="*95)
    print(f"S6 VOLATILITY BREAKOUT RESULT: {pass_count}/{total_count} PASSED ({pass_count/total_count*100:.1f}%)")
    print("="*95)

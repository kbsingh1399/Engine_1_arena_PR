"""
================================================================================
ENGINE 2: S12 ADAPTIVE REGIME STRATEGY
================================================================================
Regime-Adaptive Strategy using S2's proven architecture:
  1. Market Regime Detection (Trending/Ranging/Volatile)
  2. Regime-Specific Archetype Selection
  3. S2's Proven Momentum/Breakout Signals
  4. Per-Window IS Calibration (Zero Lookahead)
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_s12_regime")
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. DATA PREPARATION & FEATURE EXTRACTION (S2 PROVEN FEATURES)
# ─────────────────────────────────────────────────────────────────────────────
def zs(s, w):
...
def load_and_preprocess_data():
    """Loads datasets and generates S2's proven feature set."""
    logger.info("Loading 18-asset historical parquet datasets for S12 (Adaptive Regime)...")
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. REGIME-ADAPTIVE ARCHETYPE SIGNALS (S2 PROVEN + VARIATIONS)
# ─────────────────────────────────────────────────────────────────────────────
ARCHETYPE_FUNCTIONS = {
    # S2 Proven Archetypes
...
def run_all_20_windows(data_by_symbol):
    ...
    logger.info("Extracting candidate trade streams for regime-adaptive archetypes...")
    t0_ext = time.time()
    archetype_datasets = {}
    ...
    all_window_results = []
    status_file = os.path.join(RESULTS_DIR, "s12_status.json")
    ...
    logger.info("🎉 PASSED ALL 20 OUT-OF-SAMPLE WINDOWS SEQUENTIALLY FOR S12!")
    return True
...
def run_autonomous_loop():
    """Executes S12 walk-forward optimization."""
    logger.info("Initializing Autonomous 20-Window OOS Optimization Loop for S12 (Adaptive Regime)...")
    ...
    if success:
        ...
        with open(result_path, "w") as f:
            json.dump({
                "strategy": "S12_Adaptive_Regime_Strategy",
                "horizon_months": 18,
                "concurrency": 2,
                "leverage": 10.0,
        ...
        print("\n" + "="*80, flush=True)
        print("🏆 S12 CONQUERED — ALL 20 WINDOWS PASSED", flush=True)
        print("="*80 + "\n", flush=True)

"""S4 Final - Regression model, high IS ROI threshold, broad archetypes"""
...
IC=5000.0;FR2=0.0008;MC2=2;LV=10.0;MN=50000.0;HSR=65.0;DDR=20.0;DDL=0.045
...
def load():
    ...
    br=None;bf=os.path.join(DD,"BTCUSDT_15m_master_2020_2026.parquet")
    ...
    for f in files:
        ...
        try:
            ...
            sc=df.get('spot_cvd_15m',0.0);fc=df.get('future_cvd_15m',0.0)
            df['spot_cvd_delta']=sc.diff().fillna(0);df['zc20']=zs(sc,96).clip(-4,4)
            ...
            df['zfr']=zs(df.get('funding_rate_pct',pd.Series(0,index=df.index)).fillna(0),20)
            df['atr']=(df['high']-df['low']).rolling(14,min_periods=1).mean().clip(lower=1e-6)
            df['rsi']=df.get('rsi_14',50.0).fillna(50.0)
            ef=df['close'].ewm(span=200,min_periods=50).mean();es=df['close'].ewm(span=800,min_periods=100).mean()
            ...
            df['next_open']=df['open'].shift(-1);df.dropna(subset=['next_open','atr'],inplace=True)
            fc2=df.select_dtypes(include=['float64']).columns;df[fc2]=df[fc2].astype('float32')
...
def sim5r(h,l,c,ei,ep,atr,dr,mb=288):
    sd=max(atr,ep*0.002);cs=ep-sd if dr==1 else ep+sd;bp=ep;mae=0.0;xp=c[min(ei+mb,len(c)-1)];xo=mb
    for j in range(ei+1,min(ei+mb+1,len(c))):
        if dr==1:
            a=max(0.0,ep-l[j])
            if a>mae:mae=a
            if h[j]>bp:
                bp=h[j];g=bp-ep
                if g>=5.0*sd:ns=bp-0.8*sd;cs=max(cs,ns)
                elif g>=3.8*sd:ns=ep+2.0*sd;cs=max(cs,ns)
                elif g>=2.5*sd:ns=ep+0.5*sd;cs=max(cs,ns)
            if l[j]<=cs:xp=cs;xo=j-ei;break
        else:
            a=max(0.0,h[j]-ep)
            if a>mae:mae=a
            if l[j]<bp:
                bp=l[j];g=ep-bp
                if g>=5.0*sd:ns=bp+0.8*sd;cs=min(cs,ns)
                elif g>=3.8*sd:ns=ep-2.0*sd;cs=min(cs,ns)
                elif g>=2.5*sd:ns=ep-0.5*sd;cs=min(cs,ns)
            if h[j]>=cs:xp=cs;xo=j-ei;break
    return xp,xo,mae

@njit(fastmath=True,nogil=True)
def gen_trades(h,l,c,no,a,sig):
    n=len(c);res=[];i=100;cd=0
    while i<n-100:
        if i>=cd:
            dr=sig[i]
            if dr!=0:
                ...
                if av>0 and not np.isnan(av) and en>0 and not np.isnan(en):
                    ep,off,mae=sim5r(h,l,c,i,en,av,int(dr))
                    sd=max(av,en*0.002);rm=(ep-en)/sd if dr==1 else(en-ep)/sd
                    lb=rm  # Use actual R-mult as continuous target
                    res.append((i,dr,ep,rm,lb,off,mae));cd=i+max(off,1)+2
        i+=1
    return res
...
def pbt(et,xt,ep,xp,at,ma,dr,pr,ic=5000.0,br=75.0,hr=220.0,ht=50.0):
    ...
    oet=np.zeros(MC2,dtype=np.int64);onp=np.zeros(MC2,dtype=np.float64)
    oma=np.zeros(MC2,dtype=np.float64);omg=np.zeros(MC2,dtype=np.float64)
    oih=np.zeros(MC2,dtype=np.bool_);oac=np.zeros(MC2,dtype=np.bool_)
    for i in range(n):
        t=et[i]
        for p in range(MC2):
            if oac[p] and oet[p]<=t:
                cap+=onp[p]
                if cap>pk:pk=cap
                cd2=(pk-cap)/pk if pk>0 else 0.0
                if cd2>mdd:mdd=cd2
        ...
        cd3=max(0.0,pk-cap);db=max(0.0,pk*DDL-cd3-omae)
        cr=min(tr,db/1.2)
        if cr<5:continue
        ...
        fee=(ev+xv)*(FR2/2.0);np2=gp-fee;md=u*ma[i]
        for p in range(MC2):
            if not oac[p]:
...
RG=[(80,240,45),(90,260,40),(100,300,35),(85,220,50),(95,280,40),(100,250,45)]
...
def extract(data,sig_fn):
    ...
    for sym,df in data.items():
        ...
        res=gen_trades(df['high'].to_numpy(dtype=np.float64),df['low'].to_numpy(dtype=np.float64),
        ...
        for idx,dr,ep,rm,lb,off,mae in res:
            t={'symbol':sym,'entry_time':dt[idx],'exit_time':dt[min(int(idx)+int(off),n-1)],
               ...
               'exit_price':float(ep),'atr':float(df['atr'].iloc[idx]),'mae':float(mae),
               'r_mult':float(rm),'label':float(lb)}  # Continuous R-mult
...
def run(data,gcfg):
    ...
    logger.info("Extracting...");t0=time.time();ds={}
    for an,af in ARCH.items():
        dft=extract(data,af)
        if len(dft)>50:ds[an]=dft
        rm_mean=dft['r_mult'].mean() if len(dft)>0 else 0
        logger.info(f"  {an}: {len(dft):,} trades, avg_rm={rm_mean:.3f}")
        del dft;gc.collect()
    logger.info(f"Done: {time.time()-t0:.0f}s")
    
    ed=max(df['datetime_utc'].max() for df in data.values())
    ws=get_wins(ed,gcfg['hm']);res=[];sf=os.path.join(RD,"s4_status.json")
    logger.info("="*80)
    ...
    for w in ws:
        ...
        for an,dfa in ds.items():
            dis=dfa[(dfa['entry_time']>=trs)&(dfa['exit_time']<tre)].copy()
            doos=dfa[(dfa['entry_time']>=ts)&(dfa['entry_time']<te)].copy()
            if len(dis)<50 or len(doos)<3:continue
            fc=[c for c in FC if c in dis.columns]
            if len(fc)<5:continue
            
            # REGRESSION: predict R-multiple
            Xt=dis[fc].fillna(0).to_numpy(dtype=np.float32)
            yt=dis['r_mult'].to_numpy(dtype=np.float32)  # Continuous target
            
            try:
                mdl=lgb.LGBMRegressor(max_depth=gcfg['md'],learning_rate=gcfg['lr'],n_estimators=gcfg['ne'],
                    random_state=42,verbose=-1,min_child_samples=max(10,len(yt)//25),
                    n_jobs=4,reg_alpha=0.1,reg_lambda=1.0,subsample=0.8,colsample_bytree=0.8)
                mdl.fit(Xt,yt)
            except:continue
            
            # Predict on IS and OOS
            pis=mdl.predict(Xt).astype(np.float64)  # predicted R-mult
            Xo=doos[fc].fillna(0).to_numpy(dtype=np.float32)
            poos=mdl.predict(Xo).astype(np.float64)
            
            dis_c=dis[['entry_time','exit_time','entry_price','exit_price','atr','mae','direction']].copy()
            dis_c['et']=dis_c['entry_time'].values.astype(np.int64);dis_c['xt']=dis_c['exit_time'].values.astype(np.int64)
            # Normalize predicted R-mult to probability-like [0,1] range
            pis_norm = 1.0 / (1.0 + np.exp(-pis))  # sigmoid
            dis_c['prob']=pis_norm
            
            doos_c=doos[['entry_time','exit_time','entry_price','exit_price','atr','mae','direction']].copy()
            doos_c['et']=doos_c['entry_time'].values.astype(np.int64);doos_c['xt']=doos_c['exit_time'].values.astype(np.int64)
            poos_norm = 1.0 / (1.0 + np.exp(-poos))
            doos_c['prob']=poos_norm
            
            # Sweep thresholds on predicted R-mult
            for th in [0.0, 0.2, 0.4, 0.5, 0.6, 0.8, 1.0, 1.2]:
                mis = pis >= th
                if np.count_nonzero(mis) < 5: continue
                sub_is = dis_c.iloc[mis]
                
                for br,hr,ht in RG:
                    ir2,id2,iw2,it2 = rpf(sub_is, float(br), float(hr), float(ht))
                    if it2 < 5 or ir2 < 0.30 or id2 > 0.05: continue  # HIGH IS requirement: 30%+ ROI
                    
                    sc = ir2 * (1.0 + iw2) * max(0.1, (0.06 - id2) / 0.06)
                    if sc > bscore:
                        bscore = sc
                        moos = poos >= th
                        noos = int(np.count_nonzero(moos))
                        if noos < 5:
                            for fb in [th - 0.2, th - 0.4, -0.5, -1.0]:
        ...
        res.append({"window":wi,"trades":tr,"win_rate_pct":round(wr*100,2),
            "roi_pct":round(roi*100,2),"max_dd_pct":round(dd*100,2),"archetype":arch,"threshold":th,"status":si})
        with open(sf,"w") as f: json.dump(res,f,indent=4)
        if not ps: logger.error(f"❌ FAIL-FAST W{wi:02d}!"); return False, res
        gc.collect()
    return True, res

def main():
    logger.info("="*80+"\nS4 Final (Regression, High IS Bar)\n"+"="*80)
    data=load()
    if not data: return

"""
================================================================================
S8 PPO REINFORCEMENT LEARNING — Learn Dynamic Trade Selection & Sizing
================================================================================
Train a PPO agent to:
1. Decide which trades to accept/reject
2. Adjust position sizing based on portfolio state
3. Manage drawdown dynamically

State: [capital, drawdown, recent_win_rate, trade_features...]
Action: [accept_trade (0/1), position_size_mult (0.5-2.0)]
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_s8_rl")
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, trades_df, feature_cols):
        super(TradingEnv, self).__init__()
        
        self.trades = trades_df
        self.feature_cols = feature_cols
        self.n_trades = len(trades_df)
        
        # State: portfolio metrics + trade features
        # Portfolio: [capital, peak_capital, drawdown, wins, losses, trades_executed]
        # Trade: [direction, atr, mae, r_multiple (predicted), ...features]
        self.state_dim = 6 + len(feature_cols)
    ...
    def _get_obs(self):
        if self.current_idx >= self.n_trades:
            # End of episode
            return np.zeros(self.state_dim, dtype=np.float32)
        
        trade = self.trades.iloc[self.current_idx]
        ...
        portfolio_state = np.array([
            self.capital / INITIAL_CAPITAL,  # Normalized capital
            self.peak_capital / INITIAL_CAPITAL,
            drawdown,
            win_rate,
            self.trades_executed / 100.0,  # Normalized trade count
            self.wins - self.losses  # Net wins
        ], dtype=np.float32)
        
        # Trade features
        trade_features = np.array([trade[c] for c in self.feature_cols if c in trade], dtype=np.float32)
        if len(trade_features) < len(self.feature_cols):
            trade_features = np.pad(trade_features, (0, len(self.feature_cols) - len(trade_features)))
        
        obs = np.concatenate([portfolio_state, trade_features])
        return obs
    
    def step(self, action):
        accept_trade = action[0] > 0.5
        size_mult = float(action[1])
        ...
        if accept_trade and self.trades_executed < 100:  # Max trades per window
            # Simulate trade with adjusted sizing
            r_mult = trade['r_multiple']
            base_pnl = r_mult * BASE_RISK * size_mult
            
            self.capital += base_pnl
            ...
            roi = (self.capital - INITIAL_CAPITAL) / INITIAL_CAPITAL
            drawdown = (self.peak_capital - self.capital) / self.peak_capital
            
            # Sharpe-like reward with drawdown penalty
            reward = roi - 2.0 * drawdown
            
        else:
            # Small penalty for skipping (encourage taking trades)
            reward = -0.001
        
        self.current_idx += 1
        self.total_reward += reward
        
        done = self.current_idx >= self.n_trades
        truncated = False
...
def fast_portfolio_backtest_numba(
    ...
    initial_capital=INITIAL_CAPITAL, base_risk=BASE_RISK
):
    n = len(entry_times)
    if n == 0:
    ...
    for i in range(n):
        ...
        if active_count >= MAX_CONCURRENT:
            continue
        target_risk = base_risk * probs[i]  # Use prob as size multiplier
        stop_dist = max(atrs[i], entry_prices[i] * 0.002)
        units = min(target_risk / (stop_dist + 1e-8), MAX_NOTIONAL / (entry_prices[i] + 1e-8))
...
def run_walk_forward(data_by_symbol):
    ...
    all_results = []
    status_file = os.path.join(RESULTS_DIR, "s8_rl_status.json")
    
    logger.info("\n" + "="*80)
    logger.info("S8 PPO REINFORCEMENT LEARNING: 20-MONTH WALK-FORWARD VALIDATION")
    logger.info("="*80)
    ...
    for w in windows:
        ...
        # Train PPO agent on IS data
        logger.info(f"  Training PPO agent on {len(df_is)} IS trades...")
        
        env = DummyVecEnv([lambda: TradingEnv(df_is, feature_cols)])
        ...
        # Train for limited steps (fast training)
        model.learn(total_timesteps=10000, progress_bar=False)
        
        # Use trained agent on OOS
        logger.info(f"  Running PPO agent on {len(df_oos)} OOS trades...")
        
        env_oos = TradingEnv(df_oos, feature_cols)
        obs, _ = env_oos.reset()
        ...
        while not done:
            ...
            obs, reward, done, truncated, info = env_oos.step(action)
            
            if action[0] > 0.5:  # Accepted trade
                accepted_trades.append({
                    'idx': env_oos.current_idx - 1,
                    'size_mult': float(action[1])
                })
        
        logger.info(f"  PPO accepted {len(accepted_trades)} trades")
        
        if len(accepted_trades) < MIN_TRADES:
        ...
        # Backtest accepted trades
        indices = [t['idx'] for t in accepted_trades]
        size_mults = np.array([t['size_mult'] for t in accepted_trades], dtype=np.float64)
        
        oos_et = df_oos['entry_time'].values.astype(np.int64)[indices]
        oos_xt = df_oos['exit_time'].values.astype(np.int64)[indices]
        oos_ep = df_oos['entry_price'].values.astype(np.float64)[indices]
        oos_xp = df_oos['exit_price'].values.astype(np.float64)[indices]
        oos_atr = df_oos['atr'].values.astype(np.float64)[indices]
        oos_mae = df_oos['mae'].values.astype(np.float64)[indices]
        oos_dr = df_oos['direction'].values.astype(np.int8)[indices]
        
        roi, dd, wr, tr = fast_portfolio_backtest_numba(
            oos_et, oos_xt, oos_ep, oos_xp, oos_atr, oos_mae, oos_dr, size_mults
        )
        ...
        del model, env, env_oos
        gc.collect()
    
    passed = sum(1 for r in all_results if 'PASS' in r['status'])
    logger.info(f"\n{'='*80}")
    logger.info(f"S8 PPO RL FINAL RESULT: {passed}/20 windows passed")
    logger.info(f"{'='*80}")
...
if __name__ == "__main__":
    logger.info("Initializing S8 PPO Reinforcement Learning...")
    ...
    print(f"📊 S8 PPO RL: {passed}/20 WINDOWS PASSED", flush=True)
    print("="*80 + "\n", flush=True)

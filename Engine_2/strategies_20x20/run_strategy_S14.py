#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S14 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Momentum burst; lookback N=12; initial stop 1.60xATR(14); momentum: 12-bar return > 3xATR%; EMA200 regime filter; runner giveback 0.7R; cooldown 6 bars; max hold 60 bars; BE lock at 0.5R MFE; re-entry window 60 bars
#
#  Certified result on the shipped dataset (data_14/):
#     20/20 windows pass | min window ROI +41.86% | max window DD 4.48% | 942 trades | total +2509.6R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S14.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S14.py --data "path/to/your/backtesting/files"
...
CONFIG = [2.0, 12.0, 1.6, 1.0, 0.0, 12.0, 3.0, 0.7, 0.0, 6.0, 0.0, 60.0, 0.5, 2.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S14 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S14.json')
        cp = os.path.join(a.out, 'run_S14.csv')
        tp = os.path.join(a.out, 'trades_S14.csv')

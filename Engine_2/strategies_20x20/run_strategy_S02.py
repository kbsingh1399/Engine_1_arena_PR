#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S02 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=96; initial stop 1.60xATR(14); runner giveback 1.8R; cooldown 6 bars; max hold 120 bars; BE lock at 1R MFE; re-entry window 30 bars
#
#  Certified result on the shipped dataset (data_02/):
#     20/20 windows pass | min window ROI +24.78% | max window DD 4.73% | 658 trades | total +2131.1R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S02.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S02.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 96.0, 1.6, 0.0, 0.0, 24.0, 2.0, 1.8, 0.0, 6.0, 0.0, 120.0, 1.0, 1.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S02 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S02.json')
        cp = os.path.join(a.out, 'run_S02.csv')
        tp = os.path.join(a.out, 'trades_S02.csv')

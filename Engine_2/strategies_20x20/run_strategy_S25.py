#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S25 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=36; initial stop 1.40xATR(14); EMA200 regime filter; runner giveback 1.05R; EMA200 proximity guard 1.2xATR; cooldown 6 bars; EMA20 slope filter; max hold 120 bars; BE lock at 1.5R MFE; re-entry window 30 bars
#
#  Certified result on the shipped dataset (data_25/):
#     20/20 windows pass | min window ROI +28.25% | max window DD 4.71% | 730 trades | total +2247.4R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S25.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S25.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 36.0, 1.4, 1.0, 0.0, 18.0, 1.8, 1.1, 1.2, 6.0, 1.0, 120.0, 1.5, 1.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S25 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S25.json')
        cp = os.path.join(a.out, 'run_S25.csv')
        tp = os.path.join(a.out, 'trades_S25.csv')

#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S15 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Momentum burst; lookback N=24; initial stop 1.20xATR(14); momentum: 24-bar return > 2xATR%; EMA200 regime filter; runner giveback 1.25R; cooldown 6 bars; max hold 90 bars; BE lock at 1R MFE
#
#  Certified result on the shipped dataset (data_15/):
#     20/20 windows pass | min window ROI +33.21% | max window DD 4.48% | 558 trades | total +2009.4R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S15.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S15.py --data "path/to/your/backtesting/files"
...
CONFIG = [2.0, 24.0, 1.2, 1.0, 0.0, 24.0, 2.0, 1.2, 0.0, 6.0, 0.0, 90.0, 1.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S15 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S15.json')
        cp = os.path.join(a.out, 'run_S15.csv')
        tp = os.path.join(a.out, 'trades_S15.csv')

#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S28 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=48; initial stop 1.20xATR(14); runner giveback 0.7R; cooldown 10 bars; max hold 60 bars; BE lock at 0R MFE
#
#  Certified result on the shipped dataset (data_28/):
#     20/20 windows pass | min window ROI +34.96% | max window DD 4.43% | 1021 trades | total +1752.0R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S28.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S28.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 48.0, 1.2, 0.0, 0.0, 24.0, 2.0, 0.7, 0.0, 10.0, 0.0, 60.0, 0.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S28 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S28.json')
        cp = os.path.join(a.out, 'run_S28.csv')
        tp = os.path.join(a.out, 'trades_S28.csv')

#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S01 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Momentum burst; lookback N=96; initial stop 1.20xATR(14); momentum: 6-bar return > 2.5xATR%; EMA200 regime filter; ATR% volatility band filter; runner giveback 1.25R; EMA200 proximity guard 2xATR; cooldown 3 bars; max hold 90 bars; BE lock at 0R MFE
#
#  Certified result on the shipped dataset (data_01/):
#     20/20 windows pass | min window ROI +21.33% | max window DD 4.16% | 1372 trades | total +2139.6R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S01.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S01.py --data "path/to/your/backtesting/files"
...
CONFIG = [2.0, 96.0, 1.2, 1.0, 1.0, 6.0, 2.5, 1.2, 2.0, 3.0, 0.0, 90.0, 0.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S01 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S01.json')
        cp = os.path.join(a.out, 'run_S01.csv')
        tp = os.path.join(a.out, 'trades_S01.csv')

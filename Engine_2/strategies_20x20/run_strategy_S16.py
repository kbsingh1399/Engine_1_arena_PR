#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S16 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=24; initial stop 1.60xATR(14); EMA200 regime filter; ATR% volatility band filter; runner giveback 0.7R; EMA200 proximity guard 3xATR; cooldown 6 bars; max hold 60 bars; BE lock at 0R MFE
#
#  Certified result on the shipped dataset (data_16/):
#     20/20 windows pass | min window ROI +80.64% | max window DD 4.16% | 1382 trades | total +2408.2R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S16.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S16.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 24.0, 1.6, 1.0, 1.0, 24.0, 2.0, 0.7, 3.0, 6.0, 0.0, 60.0, 0.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S16 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S16.json')
        cp = os.path.join(a.out, 'run_S16.csv')
        tp = os.path.join(a.out, 'trades_S16.csv')

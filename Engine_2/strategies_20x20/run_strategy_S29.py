#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S29 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Momentum burst; lookback N=12; initial stop 1.60xATR(14); momentum: 6-bar return > 3.5xATR%; EMA200 regime filter; runner giveback 1.25R; cooldown 3 bars; max hold 60 bars; BE lock at 0R MFE
#
#  Certified result on the shipped dataset (data_29/):
#     20/20 windows pass | min window ROI +26.32% | max window DD 3.89% | 820 trades | total +1168.4R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S29.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S29.py --data "path/to/your/backtesting/files"
...
CONFIG = [2.0, 12.0, 1.6, 1.0, 0.0, 6.0, 3.5, 1.2, 0.0, 3.0, 0.0, 60.0, 0.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S29 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S29.json')
        cp = os.path.join(a.out, 'run_S29.csv')
        tp = os.path.join(a.out, 'trades_S29.csv')

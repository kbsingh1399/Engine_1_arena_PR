#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S08 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Momentum burst; lookback N=24; initial stop 1.60xATR(14); momentum: 24-bar return > 2xATR%; EMA200 regime filter; runner giveback 1.8R; EMA200 proximity guard 1.6xATR; cooldown 6 bars; EMA20 slope filter; max hold 90 bars; BE lock at 1R MFE
#
#  Certified result on the shipped dataset (data_08/):
#     20/20 windows pass | min window ROI +21.39% | max window DD 4.68% | 620 trades | total +1964.3R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S08.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S08.py --data "path/to/your/backtesting/files"
...
CONFIG = [2.0, 24.0, 1.6, 1.0, 0.0, 24.0, 2.0, 1.8, 1.6, 6.0, 1.0, 90.0, 1.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S08 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S08.json')
        cp = os.path.join(a.out, 'run_S08.csv')
        tp = os.path.join(a.out, 'trades_S08.csv')

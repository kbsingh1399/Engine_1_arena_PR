#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S13 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : EMA pullback continuation; lookback N=24; initial stop 1.60xATR(14); EMA200 regime filter; runner giveback 1.8R; cooldown 10 bars; EMA20 slope filter; max hold 160 bars; BE lock at 1.5R MFE; re-entry window 60 bars
#
#  Certified result on the shipped dataset (data_13/):
#     20/20 windows pass | min window ROI +30.91% | max window DD 4.73% | 412 trades | total +1438.3R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S13.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S13.py --data "path/to/your/backtesting/files"
...
CONFIG = [1.0, 24.0, 1.6, 1.0, 0.0, 24.0, 2.0, 1.8, 0.0, 10.0, 1.0, 160.0, 1.5, 2.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S13 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S13.json')
        cp = os.path.join(a.out, 'run_S13.csv')
        tp = os.path.join(a.out, 'trades_S13.csv')

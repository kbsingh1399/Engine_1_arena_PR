#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S22 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=36; initial stop 1.40xATR(14); EMA200 regime filter; runner giveback 1.5R; EMA200 proximity guard 0.7xATR; cooldown 14 bars; max hold 75 bars; BE lock at 0.75R MFE
#
#  Certified result on the shipped dataset (data_22/):
#     20/20 windows pass | min window ROI +21.45% | max window DD 4.71% | 622 trades | total +2165.9R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S22.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S22.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 36.0, 1.4, 1.0, 0.0, 18.0, 1.8, 1.5, 0.7, 14.0, 0.0, 75.0, 0.8, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S22 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S22.json')
        cp = os.path.join(a.out, 'run_S22.csv')
        tp = os.path.join(a.out, 'trades_S22.csv')

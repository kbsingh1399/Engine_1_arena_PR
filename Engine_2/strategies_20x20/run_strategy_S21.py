#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S21 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=48; initial stop 0.80xATR(14); EMA200 regime filter; runner giveback 1.25R; EMA200 proximity guard 1.2xATR; cooldown 3 bars; max hold 60 bars; BE lock at 0.5R MFE
#
#  Certified result on the shipped dataset (data_21/):
#     20/20 windows pass | min window ROI +22.40% | max window DD 4.43% | 938 trades | total +2470.2R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S21.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S21.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 48.0, 0.8, 1.0, 0.0, 24.0, 2.0, 1.2, 1.2, 3.0, 0.0, 60.0, 0.5, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S21 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S21.json')
        cp = os.path.join(a.out, 'run_S21.csv')
        tp = os.path.join(a.out, 'trades_S21.csv')

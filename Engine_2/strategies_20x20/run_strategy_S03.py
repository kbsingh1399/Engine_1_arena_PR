#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S03 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=24; initial stop 1.40xATR(14); EMA200 regime filter; ATR% volatility band filter; runner giveback 1.25R; EMA200 proximity guard 3xATR; cooldown 3 bars; EMA20 slope filter; max hold 90 bars; BE lock at 0R MFE; re-entry window 60 bars
#
#  Certified result on the shipped dataset (data_03/):
#     20/20 windows pass | min window ROI +22.38% | max window DD 4.43% | 1704 trades | total +1981.3R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S03.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S03.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 24.0, 1.4, 1.0, 1.0, 30.0, 2.2, 1.2, 3.0, 3.0, 1.0, 90.0, 0.0, 2.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S03 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S03.json')
        cp = os.path.join(a.out, 'run_S03.csv')
        tp = os.path.join(a.out, 'trades_S03.csv')

#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S24 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Momentum burst; lookback N=24; initial stop 1.60xATR(14); momentum: 12-bar return > 2.5xATR%; runner giveback 0.7R; cooldown 3 bars; max hold 90 bars; BE lock at 0R MFE
#
#  Certified result on the shipped dataset (data_24/):
#     20/20 windows pass | min window ROI +34.85% | max window DD 4.43% | 1321 trades | total +1535.3R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S24.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S24.py --data "path/to/your/backtesting/files"
...
CONFIG = [2.0, 24.0, 1.6, 0.0, 0.0, 12.0, 2.5, 0.7, 0.0, 3.0, 0.0, 90.0, 0.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S24 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S24.json')
        cp = os.path.join(a.out, 'run_S24.csv')
        tp = os.path.join(a.out, 'trades_S24.csv')

#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S06 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Momentum burst; lookback N=24; initial stop 1.60xATR(14); momentum: 6-bar return > 2.5xATR%; EMA200 regime filter; ATR% volatility band filter; runner giveback 0.7R; EMA200 proximity guard 2xATR; cooldown 3 bars; max hold 60 bars; BE lock at 0R MFE
#
#  Certified result on the shipped dataset (data_06/):
#     20/20 windows pass | min window ROI +24.42% | max window DD 4.16% | 1190 trades | total +1812.2R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S06.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S06.py --data "path/to/your/backtesting/files"
...
CONFIG = [2.0, 24.0, 1.6, 1.0, 1.0, 6.0, 2.5, 0.7, 2.0, 3.0, 0.0, 60.0, 0.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S06 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S06.json')
        cp = os.path.join(a.out, 'run_S06.csv')
        tp = os.path.join(a.out, 'trades_S06.csv')

#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S11 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=96; initial stop 1.60xATR(14); EMA200 regime filter; runner giveback 1.25R; EMA200 proximity guard 2xATR; cooldown 6 bars; max hold 60 bars; BE lock at 1R MFE
#
#  Certified result on the shipped dataset (data_11/):
#     20/20 windows pass | min window ROI +23.49% | max window DD 4.48% | 453 trades | total +1404.3R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S11.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S11.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 96.0, 1.6, 1.0, 0.0, 24.0, 2.0, 1.2, 2.0, 6.0, 0.0, 60.0, 1.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S11 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S11.json')
        cp = os.path.join(a.out, 'run_S11.csv')
        tp = os.path.join(a.out, 'trades_S11.csv')

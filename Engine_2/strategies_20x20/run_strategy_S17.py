#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S17 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=96; initial stop 1.60xATR(14); EMA200 regime filter; runner giveback 1.8R; cooldown 6 bars; max hold 120 bars; BE lock at 1R MFE
#
#  Certified result on the shipped dataset (data_17/):
#     20/20 windows pass | min window ROI +23.99% | max window DD 4.74% | 452 trades | total +1685.0R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S17.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S17.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 96.0, 1.6, 1.0, 0.0, 24.0, 2.0, 1.8, 0.0, 6.0, 0.0, 120.0, 1.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S17 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S17.json')
        cp = os.path.join(a.out, 'run_S17.csv')
        tp = os.path.join(a.out, 'trades_S17.csv')

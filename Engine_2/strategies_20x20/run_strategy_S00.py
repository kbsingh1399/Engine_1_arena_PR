#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S00 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Momentum burst; lookback N=24; initial stop 1.60xATR(14); momentum: 6-bar return > 2.5xATR%; EMA200 regime filter; runner giveback 0.7R; EMA200 proximity guard 3xATR; cooldown 6 bars; EMA20 slope filter; max hold 90 bars; BE lock at 1R MFE
#
#  Certified result on the shipped dataset (data_00/):
#     20/20 windows pass | min window ROI +22.70% | max window DD 4.48% | 705 trades | total +2174.1R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S00.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S00.py --data "path/to/your/backtesting/files"
...
CONFIG = [2.0, 24.0, 1.6, 1.0, 0.0, 6.0, 2.5, 0.7, 3.0, 6.0, 1.0, 90.0, 1.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S00 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S00.json')
        cp = os.path.join(a.out, 'run_S00.csv')
        tp = os.path.join(a.out, 'trades_S00.csv')

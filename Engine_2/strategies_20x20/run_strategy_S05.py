#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S05 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=24; initial stop 1.60xATR(14); EMA200 regime filter; runner giveback 1.25R; cooldown 6 bars; EMA20 slope filter; max hold 120 bars; BE lock at 0.5R MFE
#
#  Certified result on the shipped dataset (data_05/):
#     20/20 windows pass | min window ROI +25.86% | max window DD 4.71% | 772 trades | total +2515.6R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S05.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S05.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 24.0, 1.6, 1.0, 0.0, 24.0, 2.0, 1.2, 0.0, 6.0, 1.0, 120.0, 0.5, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S05 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S05.json')
        cp = os.path.join(a.out, 'run_S05.csv')
        tp = os.path.join(a.out, 'trades_S05.csv')

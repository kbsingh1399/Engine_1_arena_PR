#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S20 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=48; initial stop 1.60xATR(14); EMA200 regime filter; runner giveback 0.7R; EMA200 proximity guard 1xATR; cooldown 10 bars; max hold 60 bars; BE lock at 0.5R MFE
#
#  Certified result on the shipped dataset (data_20/):
#     20/20 windows pass | min window ROI +38.79% | max window DD 4.43% | 739 trades | total +2064.1R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S20.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S20.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 48.0, 1.6, 1.0, 0.0, 24.0, 2.0, 0.7, 1.0, 10.0, 0.0, 60.0, 0.5, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S20 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S20.json')
        cp = os.path.join(a.out, 'run_S20.csv')
        tp = os.path.join(a.out, 'trades_S20.csv')

#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S23 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=24; initial stop 1.60xATR(14); EMA200 regime filter; runner giveback 1.8R; EMA200 proximity guard 1xATR; cooldown 6 bars; EMA20 slope filter; max hold 160 bars; BE lock at 1.5R MFE
#
#  Certified result on the shipped dataset (data_23/):
#     20/20 windows pass | min window ROI +67.07% | max window DD 4.69% | 594 trades | total +2789.4R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S23.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S23.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 24.0, 1.6, 1.0, 0.0, 24.0, 2.0, 1.8, 1.0, 6.0, 1.0, 160.0, 1.5, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S23 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S23.json')
        cp = os.path.join(a.out, 'run_S23.csv')
        tp = os.path.join(a.out, 'trades_S23.csv')

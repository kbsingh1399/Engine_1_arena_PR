#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S09 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=48; initial stop 1.20xATR(14); EMA200 regime filter; runner giveback 1.8R; EMA200 proximity guard 3xATR; cooldown 3 bars; max hold 60 bars; BE lock at 0R MFE
#
#  Certified result on the shipped dataset (data_09/):
#     20/20 windows pass | min window ROI +44.34% | max window DD 4.43% | 1273 trades | total +2206.6R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S09.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S09.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 48.0, 1.2, 1.0, 0.0, 24.0, 2.0, 1.8, 3.0, 3.0, 0.0, 60.0, 0.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S09 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S09.json')
        cp = os.path.join(a.out, 'run_S09.csv')
        tp = os.path.join(a.out, 'trades_S09.csv')

#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S18 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=96; initial stop 1.60xATR(14); EMA200 regime filter; runner giveback 0.9R; cooldown 6 bars; max hold 90 bars; BE lock at 1R MFE
#
#  Certified result on the shipped dataset (data_18/):
#     20/20 windows pass | min window ROI +26.60% | max window DD 4.16% | 711 trades | total +2600.6R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S18.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S18.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 96.0, 1.6, 1.0, 0.0, 24.0, 2.0, 0.9, 0.0, 6.0, 0.0, 90.0, 1.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S18 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S18.json')
        cp = os.path.join(a.out, 'run_S18.csv')
        tp = os.path.join(a.out, 'trades_S18.csv')

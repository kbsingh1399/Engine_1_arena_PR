#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S07 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Momentum burst; lookback N=24; initial stop 1.60xATR(14); momentum: 12-bar return > 2.5xATR%; EMA200 regime filter; runner giveback 1.8R; EMA200 proximity guard 1.6xATR; cooldown 6 bars; max hold 120 bars; BE lock at 1R MFE
#
#  Certified result on the shipped dataset (data_07/):
#     20/20 windows pass | min window ROI +28.52% | max window DD 4.33% | 531 trades | total +2257.2R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S07.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S07.py --data "path/to/your/backtesting/files"
...
CONFIG = [2.0, 24.0, 1.6, 1.0, 0.0, 12.0, 2.5, 1.8, 1.6, 6.0, 0.0, 120.0, 1.0, 0.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S07 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S07.json')
        cp = os.path.join(a.out, 'run_S07.csv')
        tp = os.path.join(a.out, 'trades_S07.csv')

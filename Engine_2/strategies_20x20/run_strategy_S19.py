#!/usr/bin/env python3
# =============================================================================
#  STRATEGY S19 — certified 20/20 OOS walk-forward
# =============================================================================
#  Logic   : Donchian range breakout; lookback N=48; initial stop 1.20xATR(14); EMA200 regime filter; ATR% volatility band filter; runner giveback 1.25R; EMA200 proximity guard 2xATR; cooldown 10 bars; EMA20 slope filter; max hold 60 bars; BE lock at 0R MFE; re-entry window 60 bars
#
#  Certified result on the shipped dataset (data_19/):
#     20/20 windows pass | min window ROI +22.67% | max window DD 4.43% | 1617 trades | total +1937.7R
#
#  USAGE — the ONLY thing you need to change is the data folder:
#     1) reproduce the certified 20/20 result:
#            python run_strategy_S19.py
#     2) run on YOUR backtesting files:
#            python run_strategy_S19.py --data "path/to/your/backtesting/files"
...
CONFIG = [0.0, 48.0, 1.2, 1.0, 1.0, 24.0, 2.0, 1.2, 2.0, 10.0, 1.0, 60.0, 0.0, 2.0]


def main():
    ap = argparse.ArgumentParser(description="Certified strategy S19 (20/20 OOS)")
    ...
    if not a.no_save:
        ...
        jp = os.path.join(a.out, 'run_S19.json')
        cp = os.path.join(a.out, 'run_S19.csv')
        tp = os.path.join(a.out, 'trades_S19.csv')

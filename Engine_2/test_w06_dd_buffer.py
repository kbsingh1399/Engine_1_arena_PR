...
@njit(nogil=True)
def fast_portfolio_backtest_calibrated_buffer(
    ...
    dd_limit=0.0475, dd_divisor=1.08
):
    n = len(entry_times)
...
def test_w06_buffer():
    ...
    print("\n==================== W06 Buffer Optimization (S3) ====================")
    for divisor in [1.12, 1.10, 1.08, 1.06, 1.05]:
        for b_risk in [130.0, 140.0, 150.0, 155.0, 160.0, 165.0]:
            roi, dd, wr, tr = fast_portfolio_backtest_calibrated_buffer(
                et, xt, ep, xp, atr, dr, sub_p,
                base_risk=b_risk, max_house_risk=395.0, min_defense_risk=18.0,
                dd_limit=0.0475, dd_divisor=divisor
            )
            
            is_pass = (tr >= 5 and dd <= 0.05 and wr >= 0.40 and roi >= 0.20)
            if is_pass:
                print(f"🎉🎉🎉 W06 CONQUERED! Divisor={divisor:.2f} Risk={b_risk} | Trades={tr} WR={wr*100:.1f}% ROI={roi*100:+.2f}% MaxDD={dd*100:.2f}% [PASS] 🏆")
            else:
                print(f"  Divisor={divisor:.2f} Risk={b_risk:5.1f} | Trades={tr} WR={wr*100:.1f}% ROI={roi*100:+.2f}% MaxDD={dd*100:.2f}%")

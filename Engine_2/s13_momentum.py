"""
================================================================================
ENGINE 2: S13 ENHANCED MOMENTUM STRATEGY
================================================================================
Enhanced Momentum Strategy with multi-factor confirmation:
  1. 12 Proven Momentum/Breakout/Reversal Archetypes
  2. Multi-Factor Signal Confirmation (CVD, Liquidation, Volume, Trend)
  ...
  8. Per-Window IS Calibrated Archetype Selection (Regime Adaptation)
================================================================================
"""
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_s13_momentum")
...
def load_and_preprocess_data():
    ...
    logger.info("Loading 18-asset historical parquet datasets for S13 Enhanced Momentum (CVD Momentum + Whale)...")
    ...
    for f in sorted(files):
        ...
        try:
            ...
            # 5. S8 Whale vs Retail Divergence Features
            ...
            df['ls_top_zscore'] = zs(ls_top, 96).clip(-4.0, 4.0)
            df['ls_global_zscore'] = zs(ls_global, 96).clip(-4.0, 4.0)
            
            # Whale Index (composite of large trader activity)
            whale_index = (ls_top.clip(0.5, 2.0) - 1.0) * 50 + 50  # Normalize to 0-100
            df['whale_index'] = whale_index.clip(0, 100)
            
            # Order Book Depth Imbalance
            bid_depth = df.get('bid_depth_usd', pd.Series(1e6, index=df.index)).ffill().fillna(1e6)
            ask_depth = df.get('ask_depth_usd', pd.Series(1e6, index=df.index)).ffill().fillna(1e6)
            df['bid_depth_usd'] = bid_depth
            df['ask_depth_usd'] = ask_depth
            df['depth_imbalance'] = (bid_depth - ask_depth) / (bid_depth + ask_depth + 1e-8)
            df['depth_imbalance_zscore'] = zs(df['depth_imbalance'], 96).clip(-4.0, 4.0)
            
            # Whale vs Retail Divergence
            df['whale_retail_div'] = df['ls_top_zscore'] - df['ls_global_zscore']
            df['whale_momentum'] = df['whale_index'].diff().fillna(0)
            df['whale_accumulation'] = zs(df['whale_retail_div'].rolling(20, min_periods=5).sum(), 96).clip(-4.0, 4.0)
    ...
    logger.info(f"Loaded Clean Partitioned Datasets for S13 Enhanced Momentum: {total_rows:,} rows across {len(data_by_symbol)} symbols")
...
def run_all_20_windows(data_by_symbol):
    ...
    all_window_results = []
    status_file = os.path.join(RESULTS_DIR, "s13_status.json")
...
def run_autonomous_loop():
    ...
    logger.info("Initializing Autonomous 20-Window OOS Optimization Loop for S13 Enhanced Momentum (CVD Momentum)...")
    ...
    if success:
        ...
        with open(result_path, "w") as f:
            json.dump({
                "strategy": "S13_Enhanced_Momentum_CVD_Momentum_Whale",
                "horizon_months": 18,
                "concurrency": 2,
                "leverage": 10.0,
        ...
        print("\n" + "="*80, flush=True)
        print("🏆 S13 ENHANCED MOMENTUM CONQUERED — ALL 20 WINDOWS PASSED", flush=True)
        print("="*80 + "\n", flush=True)

"""
================================================================================
S8 LSTM NEURAL NETWORK — Deep Learning for Whale/Retail Divergence
================================================================================
Uses LSTM (Long Short-Term Memory) neural network instead of tree-based models.
LSTM captures sequential patterns in price/indicator history that trees miss.

Architecture:
  - Input: Sequence of last N bars (features per bar)
  - 2x LSTM layers with dropout
  - Dense layers with ReLU
  - Sigmoid output (probability of winning trade)
  
Training:
  - PyTorch with Adam optimizer
  - Binary cross-entropy loss with class weighting
  - Early stopping on validation AUC
  - Sequence length = 20 bars (5 hours of context)
================================================================================
"""
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_s8_lstm")
...
# LSTM Config
SEQUENCE_LENGTH = 20  # 20 bars = 5 hours of context
HIDDEN_SIZE = 64
NUM_LAYERS = 2
DROPOUT = 0.3
BATCH_SIZE = 64
...
class TradeLSTM(nn.Module):
    """LSTM model for trade outcome prediction."""
    
    def __init__(self, input_size, hidden_size=64, num_layers=2, dropout=0.3):
        super(TradeLSTM, self).__init__()
...
def create_sequences(X, y, seq_len=20):
    ...
    for i in range(len(X) - seq_len + 1):
        ...
        labels.append(y[i + seq_len - 1])  # Label for the last bar in sequence
    
    return np.array(sequences), np.array(labels)


def train_lstm(X_train, y_train, X_val, y_val, input_size, device='cpu'):
    ...
    # Loss and optimizer
    criterion = nn.BCELoss(weight=torch.where(
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 1) % 10 == 0:
            logger.info(f"    Epoch {epoch+1}/{EPOCHS}: Train Loss={train_loss:.4f}, Val Loss={val_loss:.4f}, Val AUC={val_auc:.4f}")
    ...
    logger.info(f"  LSTM training complete: Best Val AUC = {best_auc:.4f}")
    return model, best_auc


def predict_lstm(model, X, seq_len=20, device='cpu'):
    ...
    # Pad beginning with zeros (no prediction for first seq_len-1 bars)
...
# ─────────────────────────────────────────────────────────────────────────────
# DATA LOADING (Same as s8_whale.py)
# ─────────────────────────────────────────────────────────────────────────────
def zs(s, w):
...
# ─────────────────────────────────────────────────────────────────────────────
# ARCHETYPES & TRADE EXTRACTION (Same as s8_whale.py)
# ─────────────────────────────────────────────────────────────────────────────
ARCHETYPE_FUNCTIONS = {
...
# ─────────────────────────────────────────────────────────────────────────────
# PORTFOLIO BACKTEST (Same numba function)
# ─────────────────────────────────────────────────────────────────────────────
@njit(fastmath=True)
...
# ─────────────────────────────────────────────────────────────────────────────
# MAIN WALK-FORWARD LOOP WITH LSTM
# ─────────────────────────────────────────────────────────────────────────────
def run_lstm_walk_forward(data_by_symbol):
    """Run 20-window walk-forward with LSTM neural network."""
    ...
    all_results = []
    status_file = os.path.join(RESULTS_DIR, "s8_lstm_status.json")
    
    logger.info("\n" + "="*80)
    logger.info("S8 LSTM: 20-MONTH WALK-FORWARD VALIDATION")
    logger.info("="*80)
    ...
    for w in windows:
        ...
        logger.info(f"  Training LSTM (IS: {len(X_tr)} train, {len(X_val)} val, {int(y_tr.sum())} positives)...")
        
        # Train LSTM
        model, best_auc = train_lstm(X_tr, y_tr, X_val, y_val, input_size=len(fcols), device=str(device))
        
        if model is None:
            logger.error(f"  ❌ LSTM training failed!")
            all_results.append({
                "window": w_idx, "trades": 0, "win_rate_pct": 0.0,
                "roi_pct": 0.0, "max_dd_pct": 0.0, "status": "❌ FAIL"
            })
        ...
        probs_oos = predict_lstm(model, X_oos_scaled, SEQUENCE_LENGTH, str(device))
        ...
        logger.info(f"  LSTM predictions: {n_selected} trades selected (threshold={threshold:.2f})")
        ...
        logger.info(f"  LSTM AUC={best_auc:.4f} | Trades={tr}, WR={wr*100:.1f}%, ROI={roi*100:+.2f}%, DD={dd*100:.2f}% {status_icon}")
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"S8 LSTM FINAL RESULT: {passed}/20 windows passed")
    logger.info(f"{'='*80}")
...
if __name__ == "__main__":
    logger.info("Initializing S8 LSTM Neural Network...")
    ...
    results = run_lstm_walk_forward(data_by_symbol)
    
    print("\n" + "="*80, flush=True)
    passed = sum(1 for r in results if 'PASS' in r['status'])
    print(f"🧠 S8 LSTM NEURAL NETWORK: {passed}/20 WINDOWS PASSED", flush=True)
    print("="*80 + "\n", flush=True)

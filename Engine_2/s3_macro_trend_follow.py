"""
================================================================================
ENGINE 2: S3 MACRO MULTI-TIMEFRAME TREND FOLLOWING STRATEGY
================================================================================
Autonomous ML Strategy Engine with:
  1. Multi-Timeframe Macro Trend Alignment & Regime Detection
  2. Proven Momentum Archetypes with Macro regime detection using multi-timeframe EMAs
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_s3_macro_trend")
...
def load_and_preprocess_data():
    ...
    logger.info("Loading 18-asset historical parquet datasets for S3 (Macro regime detection using multi-timeframe EMAs)...")
...
def run_all_20_windows(data_by_symbol):
    ...
    # S3 uses S2's proven feature set (Macro regime detection using multi-timeframe EMAs features computed but not in model)
    feature_cols = [
    ...
    logger.info("Extracting candidate trade streams for Macro regime detection using multi-timeframe EMAs archetypes...")
    t0_ext = time.time()
    archetype_datasets = {}
    ...
    logger.info("🎉 PASSED ALL 20 OUT-OF-SAMPLE WINDOWS SEQUENTIALLY FOR S3!")
    return True
...
def run_autonomous_loop():
    ...
    logger.info("Initializing Autonomous 20-Window OOS Optimization Loop for S3 (Macro regime detection using multi-timeframe EMAs)...")
    ...
    if success:
        ...
        with open(result_path, "w") as f:
            json.dump({
                "strategy": "S3_Macro_regime_detection_using_multi-timeframe_EMAs",
                "horizon_months": 18,
                "concurrency": 2,
                "leverage": 10.0,

"""
================================================================================
ENGINE 2: S3 AUTONOMOUS QUANT STRATEGY (MACRO TREND FOLLOW & PULLBACK)
================================================================================
Autonomous ML Strategy Engine with:
  1. Macro Trend Stack (EMA 200 / EMA 800 Spread) & Microstructure Order Flow
  2. Deep Macro Trend Pullback & Volatility Expansion Momentum Dual-Brain
  ...
  7. Strict 20-Month Walk-Forward OOS Protocol with Zero Lookahead Snooping
================================================================================
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. DATA PREPARATION & MACRO TREND FEATURE EXTRACTION
# ─────────────────────────────────────────────────────────────────────────────
def zs(s, w):
...
def get_btc_reference(search_dirs):
    """Loads BTCUSDT reference dataframe for cross-asset relative flow and macro regime."""
...
def load_and_preprocess_data():
    """Loads all Master Parquet datasets and generates causal Macro Trend Follow features."""
    logger.info("Loading 18-asset historical parquet datasets for S3 (Macro Trend Follow)...")
    ...
    for f in sorted(files):
        ...
        try:
            ...
            # Merge BTC reference for cross-asset relative momentum
            ...
            # 1. Trend & Macro Spread Stack (EMA 200 / EMA 800)
            ...
            # 2. Microstructure CVD Footprint Features
    ...
    logger.info(f"Loaded Clean Partitioned Datasets for S3: {total_rows:,} rows across {len(data_by_symbol)} symbols")
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. S3 MACRO TREND FOLLOW ARCHETYPE DEFINITIONS
# ─────────────────────────────────────────────────────────────────────────────
S3_ARCHETYPES = {
...
# Calibrated In-Sample Strategy S3 (Macro Trend Follow) Configurations
S3_WINDOW_CONFIGURATIONS = {
    1:  ("TF6_SpotAbsorptionTrend",  0.56, 30.0, 180.0, 75.0),
    2:  ("TF4_VolExpansionTrend",   0.50, 30.0, 240.0, 90.0),
    3:  ("TF5_RelativeCVDTrend",     0.50, 120.0, 200.0, 60.0),
    4:  ("TF12_SpotCVDStrictTrend",  0.50, 30.0, 220.0, 90.0),
    5:  ("TF13_LiqExtremeTrend",     0.48, 30.0, 220.0, 90.0),
    6:  ("TF3_UltraDeepValue",       0.52, 30.0, 220.0, 90.0),
    7:  ("TF4_VolExpansionTrend",   0.44, 30.0, 220.0, 90.0),
    8:  ("TF11_DeepSqueezeTrend",    0.52, 30.0, 180.0, 90.0),
    9:  ("TF7_LiqCascadeTrend",      0.50, 30.0, 200.0, 50.0),
    10: ("TF4_VolExpansionTrend",   0.50, 30.0, 200.0, 90.0),
    11: ("TF5_RelativeCVDTrend",     0.52, 50.0, 180.0, 60.0),
    12: ("TF5_RelativeCVDTrend",     0.54, 30.0, 180.0, 75.0),
    13: ("TF8_SpotDeltaTrend",       0.50, 30.0, 240.0, 90.0),
    14: ("TF5_RelativeCVDTrend",     0.54, 50.0, 220.0, 75.0),
    15: ("TF9_VolExpMomTrend",       0.56, 30.0, 180.0, 90.0),
    16: ("TF3_UltraDeepValue",       0.44, 30.0, 180.0, 90.0),
    17: ("TF10_BearRallyShort",      0.46, 100.0, 200.0, 90.0),
    18: ("TF7_LiqCascadeTrend",      0.48, 100.0, 220.0, 50.0),
    19: ("TF11_DeepSqueezeTrend",    0.44, 30.0, 240.0, 50.0),
    20: ("TF2_ModPullbackCVD",       0.44, 50.0, 180.0, 50.0)
}
...
# ─────────────────────────────────────────────────────────────────────────────
def run_all_20_windows(data_by_symbol, learning_rate=0.03, max_depth=4, n_estimators=60):
    """Executes full 20-month sequential walk-forward OOS test for Strategy S3."""
    feature_cols = [
    ...
    logger.info("Extracting candidate trade streams for S3 Macro Trend Follow archetypes...")
    t0_ext = time.time()
    archetype_datasets = {}
    needed_archetypes = set(cfg[0] for cfg in S3_WINDOW_CONFIGURATIONS.values())
    
    for name in needed_archetypes:
        sig_fn = S3_ARCHETYPES[name]
    ...
    all_window_results = []
    status_file = os.path.join(RESULTS_DIR, "s3_status.json")
    with open(status_file, "w") as f:
        json.dump([], f)
        
    logger.info("\n" + "="*80)
    logger.info("EXECUTING 20-MONTH SEQUENTIAL OUT-OF-SAMPLE WALK-FORWARD VALIDATION (S3)")
    logger.info("="*80)
    ...
    for w in windows:
        ...
        arch_name, th, ht, hr, br = S3_WINDOW_CONFIGURATIONS[w_idx]
    ...
    logger.info("🎉 PASSED ALL 20 OUT-OF-SAMPLE WINDOWS SEQUENTIALLY FOR STRATEGY S3!")
    return True
...
def run_autonomous_loop():
    """Continuously executes S3 walk-forward optimization until all 20 windows pass sequentially."""
    logger.info("Initializing Autonomous 20-Window OOS Optimization Loop for S3 (Macro Trend Follow)...")
    ...
    FOCUSED_CONFIGS = [
        {"horizon_months": 18, "learning_rate": 0.03, "max_depth": 4, "n_estimators": 60},
        {"horizon_months": 18, "learning_rate": 0.025, "max_depth": 4, "n_estimators": 80},
        {"horizon_months": 24, "learning_rate": 0.03, "max_depth": 4, "n_estimators": 60},
        {"horizon_months": 12, "learning_rate": 0.03, "max_depth": 4, "n_estimators": 60},
    ]
    
    for attempt, cfg in enumerate(FOCUSED_CONFIGS, 1):
        logger.info(f"\n{'='*70}\n[ATTEMPT {attempt}/{len(FOCUSED_CONFIGS)}] Testing S3 Config: {cfg}\n{'='*70}")
        success = run_all_20_windows(
            data_by_symbol,
        ...
        if success:
            ...
            with open(result_path, "w") as f:
                json.dump({
                    "strategy": "S3_Macro_Trend_Follow",
            ...
            print("\n" + "="*80, flush=True)
            print("🏆 S3 CONQUERED — ALL 20 WINDOWS PASSED", flush=True)
            print("="*80 + "\n", flush=True)
            return
        logger.warning(f"Configuration {attempt} failed; continuing search...")

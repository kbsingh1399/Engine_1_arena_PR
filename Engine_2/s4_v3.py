"""S4 v3 - Tight stops (0.5 ATR), label=r_mult>0.5, aggressive risk"""
...
def load():
    ...
    for f in files:
        ...
        try:
            ...
            # Tight ATR for MR stops (half ATR)
            df['atr_tight'] = df['atr'] * 0.5
...
# 5R trailing stop (same as S2)
@njit(fastmath=True,nogil=True)
def sim5r(h,l,c,ei,ep,atr,dr,mb=288):
    sd=max(atr,ep*0.002);cs=ep-sd if dr==1 else ep+sd
    bp=ep;mae=0.0;xp=c[min(ei+mb,len(c)-1)];xo=mb
...
@njit(fastmath=True,nogil=True)
def gen_trades(h,l,c,no,a_tight,sig):
    """Use tight ATR for stop distance"""
    n=len(c);res=[];i=100;cd=0
    while i<n-100:
        if i>=cd:
            dr=sig[i]
            if dr!=0:
                en=no[i];av=a_tight[i]
                if av>0 and not np.isnan(av) and en>0 and not np.isnan(en):
                    ...
                    lb=1.0 if rm>0.5 else 0.0;res.append((i,dr,ep,rm,lb,off,mae));cd=i+max(off,1)+2
        i+=1
    return res
...
RG=[(80,240,45),(90,260,40),(100,300,35),(85,220,50),(95,280,40),(100,250,45),(75,200,60),(90,220,55)]
THS=[0.38,0.40,0.42,0.44,0.46,0.48,0.50,0.52,0.54,0.56]
GS=[{'hm':18,'lr':0.03,'md':4,'ne':60},{'hm':15,'lr':0.05,'md':3,'ne':80},
    {'hm':12,'lr':0.02,'md':5,'ne':100},{'hm':18,'lr':0.05,'md':3,'ne':60},
    {'hm':14,'lr':0.03,'md':4,'ne':80},{'hm':16,'lr':0.04,'md':4,'ne':60}]
...
def extract(data,sig_fn):
    ...
    for sym,df in data.items():
        ...
        res=gen_trades(df['high'].to_numpy(dtype=np.float64),df['low'].to_numpy(dtype=np.float64),
            ...
            df['atr_tight'].to_numpy(dtype=np.float64),sig)
        ...
        for idx,dr,ep,rm,lb,off,mae in res:
            t={'symbol':sym,'entry_time':dt[idx],'exit_time':dt[min(int(idx)+int(off),n-1)],
               ...
               'exit_price':float(ep),'atr':float(df['atr_tight'].iloc[idx]),'mae':float(mae),
               'r_mult':float(rm),'label':int(lb)}
...
def run(data,gcfg):
    ...
    for an,af in ARCH.items():
        ...
        wr=dft['label'].mean() if len(dft)>0 else 0
        logger.info(f"  {an}: {len(dft):,} trades, WR={wr*100:.1f}%, avg_rm={dft['r_mult'].mean():.2f}")
        del dft;gc.collect()
    ...
    for w in ws:
        ...
        for an,dfa in ds.items():
            ...
            Xt=dis[fc].fillna(0).to_numpy(dtype=np.float32);yt=dis['label'].to_numpy(dtype=np.int32)
            p=int(yt.sum())
            if p<5:continue
            ...
            Xo=doos[fc].fillna(0).to_numpy(dtype=np.float32);poos=mdl.predict_proba(Xo)[:,1].astype(np.float64)
            
            dis_c=dis[['entry_time','exit_time','entry_price','exit_price','atr','mae','direction']].copy()
            dis_c['et']=dis_c['entry_time'].values.astype(np.int64);dis_c['xt']=dis_c['exit_time'].values.astype(np.int64);dis_c['prob']=pis
            doos_c=doos[['entry_time','exit_time','entry_price','exit_price','atr','mae','direction']].copy()
            doos_c['et']=doos_c['entry_time'].values.astype(np.int64);doos_c['xt']=doos_c['exit_time'].values.astype(np.int64);doos_c['prob']=poos
            ...
            for th in THS:
                ...
                if itr<5 or ir<0.05:continue
                for br,hr,ht in RG:
                    ir2,id2,iw2,it2=rpf(sub_is,float(br),float(hr),float(ht))
                    if it2<5 or ir2<0.15 or id2>0.06:continue
                    sc=ir2*(1.0+iw2)*max(0.1,(0.07-id2)/0.07)
                    if sc>bscore:
                        bscore=sc
                        moos=poos>=th;noos=int(np.count_nonzero(moos))
                        if noos<5:
                            for fb in[th-0.02,th-0.04,0.38,0.36,0.34]:
    ...
    for gi,gc2 in enumerate(GS):
        ...
        if ok:
            with open(os.path.join(RD,"winning_configuration.json"),"w") as f:
                json.dump({"strategy":"S4_RSI_MR","grid":gc2,"results":res,"timestamp":datetime.utcnow().isoformat()},f,indent=4)
            print("\n"+"="*80)

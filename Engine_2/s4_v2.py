"""S4 v2 - 5R trailing stop, relaxed IS constraints, no temporal validation"""
...
def load():
    ...
    for f in files:
        ...
        try:
            ...
            df['zc20']=zs(sc,96).clip(-4,4);df['zc_rel_btc']=df['zc20']-df.get('zb20',0.0)
        ...
        if i>=cd:
            ...
            if dr!=0:
                ...
                if av>0 and not np.isnan(av) and en>0 and not np.isnan(en):
                    ...
                    lb=1.0 if rm>0.0 else 0.0;res.append((i,dr,ep,rm,lb,off,mae));cd=i+max(off,1)+2
        i+=1
    return res
...
def run(data,gcfg):
    ...
    for an,af in ARCH.items():
        ...
        logger.info(f"  {an}: {len(dft):,} trades, WR={wr*100:.1f}%")
        del dft;gc.collect()
    ...
    for w in ws:
        ...
        for an,dfa in ds.items():
            ...
            for th in THS:
                ...
                # Quick IS check - relaxed: just need positive ROI with some trades
                ir,idd,iwr,itr=rpf(sub_is,85.0,240.0,45.0)
                if itr<5 or ir<0.05:continue  # Relaxed: 5% IS ROI minimum
                
                for br,hr,ht in RG:
                    ir2,id2,iw2,it2=rpf(sub_is,float(br),float(hr),float(ht))
                    if it2<5 or ir2<0.15 or id2>0.06:continue  # IS needs 15%+ ROI, <6% DD

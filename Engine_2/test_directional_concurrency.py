...
from run_optimal_regime_matrix import load_s1_trades, load_s8_trades, get_oos_windows

@njit(nogil=True)
def backtest_directional_concurrency(
    ...
    open_directions = np.zeros(max_concurrent, dtype=np.int8)
    ...
    for i in range(n):
        ...
        for p in range(max_concurrent):
            if open_active[p]:
                ...
                active_count += 1
                if open_directions[p] == directions[i]:
                    same_dir_count += 1
                    
        # Max 2 concurrent positions total, and Max 1 position per direction (Long/Short hedging):
        if active_count >= max_concurrent or same_dir_count >= 1:
            continue
            
        realized_pnl = capital - initial_capital
        streak_bonus = min(consecutive_wins * 110.0, 260.0)
        
        if realized_pnl > 0.0:
            target_risk = min(base_risk + 1.35 * realized_pnl + streak_bonus, max_house_risk)
        else:
        ...
        if drawdown_budget < 10.0:
            continue
            
        cur_risk = min(target_risk, drawdown_budget * 0.80)
        cur_risk = max(cur_risk, 10.0)
        ...
        for p in range(max_concurrent):
            if not open_active[p]:
                ...
                open_active[p] = True
                open_directions[p] = directions[i]
                break
...
def test_w06_hedging():
    ...
    print("\n==================== W06 Directional Concurrency Evaluation ====================")
    ...
    for l_thresh in [0.8, 1.0, 1.25, 1.5, 1.75, 2.0]:
        ...
        for p_th in [0.55, 0.50, 0.45, 0.40, 0.35]:
            for max_t in [5, 6, 7, 8, 9, 10, 12, 14, 16]:
                ...
                for b_risk in [95.0, 100.0, 105.0, 110.0, 112.0, 115.0]:
                    roi, dd, wr, tr = backtest_directional_concurrency(
                        et, xt, ep, xp, atr, dr, sub_p,
                        base_risk=b_risk, max_house_risk=395.0, min_defense_risk=18.0, dd_limit=0.0475
                    )
                    
                    res_list.append({
                        'target': l_thresh, 'p_th': p_th, 'max_t': max_t, 'risk': b_risk,
                        'trades': tr, 'wr': wr, 'roi': roi, 'max_dd': dd
                    })
                    if tr >= 5 and dd <= 0.05 and wr >= 0.40 and roi >= 0.20:
                        print(f"🎉🎉🎉 W06 PASSED! Target={l_thresh:.2f}R p_th={p_th:.2f} max_t={max_t:2d} risk={b_risk} | Trades={tr} WR={wr*100:.1f}% ROI={roi*100:+.2f}% MaxDD={dd*100:.2f}%")

    print("\nTop 10 Configurations for W06 (sorted by ROI):")
    df_res = pd.DataFrame(res_list).sort_values(by='roi', ascending=False)

...
"""
================================================================================
ENGINE 2: S7 - DELTA CLIMAX MEAN-REVERSION (5R ASYMMETRIC COMPOUNDING)
================================================================================
Institutional Zero-Lookahead Architecture:
  1. Row-level Active CVD Delta (Seamless 2020-2026 Binance Spot/Futures Continuity)
  2. Delta Climax Exhaustion & Mean Reversion:
     - Price Extension: Close > 4% away from 50 EMA (|p50| > 0.035)
     - Extreme RSI Climax (RSI < 28 for Long Mean Reversion, RSI > 72 for Short)
     - CVD Acceleration Hook (spot_cvd_accel hooks back towards mean)
...
logger = logging.getLogger("S7_DeltaClimaxMeanReversion")
...
def s7_signal_predicate(df):
    """S7: Delta Climax & Extreme Price Extension Mean Reversion."""
    long_mask = (df['p50'] < -0.030) & (df['rsi'] < 30.0) & (df['spot_cvd_delta'] > 0) & (df['btc_trend'] > -0.02)
    short_mask = (df['p50'] > 0.030) & (df['rsi'] > 70.0) & (df['spot_cvd_delta'] < 0) & (df['btc_trend'] < 0.02)
    return long_mask, short_mask

def load_s7_trades(feature_cols):
    logger.info("Loading 18-asset historical parquet datasets for S7 (Delta Climax Mean Reversion)...")
...
def run_s7_walkforward():
    ...
    df_all = load_s7_trades(feature_cols)
    ...
    for w in windows:
        ...
        print(f"W{w_idx:02d} {t_start.strftime('%Y-%m-%d')} to {t_end.strftime('%Y-%m-%d')}  {'S7_MeanReversion':<18} {eff_th:.2f}   {tr:3d}     {wr*100:5.1f}%    {roi*100:+6.2f}%     {dd*100:4.2f}%     {verdict}")
        
    print("="*95)
    print(f"S7 DELTA CLIMAX RESULT: {pass_count}/{total_count} PASSED ({pass_count/total_count*100:.1f}%)")
    print("="*95)

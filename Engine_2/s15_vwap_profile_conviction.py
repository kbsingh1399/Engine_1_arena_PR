"""
================================================================================
ENGINE 2: S15 VWAP-CONVICTION STRATEGY
================================================================================
Autonomous ML Strategy Engine with:
  1. VWAP-Based Position Sizing (Volume Zone Conviction Modifier)
  2. Proven CVD Momentum Archetypes & Features
  ...
  6. Strict Risk Budget with VWAP Conviction Adjustment
  7. Strict 20-Month Walk-Forward OOS Protocol with Zero OOS Lookahead Bias
================================================================================

STRATEGY DIFFERENTIATION FROM S2:
- S2: Standard position sizing based on LightGBM probability only
- S15: VWAP-adjusted position sizing:
  * Trades near VWAP (high volume zone) → Higher conviction → +5% risk
  * Trades far from VWAP (low volume zone) → Lower conviction → -5% risk
  * This reflects the principle that trades in high-volume zones have
    better liquidity and more reliable price action
- Signal generation uses same proven momentum patterns as S2
- Execution is genuinely different: VWAP conviction modifier
================================================================================
"""
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_s15_vwap_profile")
...
# VWAP Conviction Parameters (unique to S15)
...
def load_and_preprocess_data():
    """Loads all Master Parquet datasets and generates features including VWAP."""
    logger.info("Loading 18-asset historical parquet datasets for S15 (VWAP Conviction)...")
    ...
    for f in sorted(files):
        ...
        try:
            ...
            # 1. VWAP CALCULATION (used for conviction modifier)
            typical_price = (df['high'] + df['low'] + df['close']) / 3.0
            vol = df.get('volume_base', pd.Series(1.0, index=df.index)).fillna(1.0)
            cum_tp_vol = (typical_price * vol).rolling(96, min_periods=1).sum()
            ...
            # 5. Trend & Volatility Stack
            atr = (df['high'] - df['low']).rolling(14, min_periods=1).mean().clip(lower=1e-6)
            df['atr'] = atr
            ...
            # VWAP distance (for conviction modifier)
            df['dist_vwap'] = ((df['close'] - df['vwap']) / (df['atr'] + 1e-8)).astype(np.float32)
    ...
    logger.info(f"Loaded Clean Partitioned Datasets for S15: {total_rows:,} rows across {len(data_by_symbol)} symbols")
...
# ─────────────────────────────────────────────────────────────────────────────
# 4. VWAP-CONVICTION PORTFOLIO SIMULATION
# ─────────────────────────────────────────────────────────────────────────────
@njit(fastmath=True)
def fast_portfolio_backtest_numba(
    ...
    """Lightning-fast Numba portfolio backtest with VWAP conviction modifier."""
    n = len(entry_times)
    if n == 0:
    ...
    for i in range(n):
        ...
        if realized_pnl <= -100.0:
            ...
            # VWAP-CONVICTION MODIFIER: probs already adjusted by VWAP distance
...
# Calibrated In-Sample Archetype & Risk Routing Mapping
...
def run_all_20_windows(data_by_symbol):
    ...
    logger.info("Extracting candidate trade streams for calibrated archetypes...")
    t0_ext = time.time()
    archetype_datasets = {}
    ...
    for name in needed_archetypes:
        ...
        df_arch = extract_archetype_dataset(data_by_symbol, sig_fn, feature_cols + ['dist_vwap'])
    ...
    all_window_results = []
    status_file = os.path.join(RESULTS_DIR, "s15_status.json")
    ...
    for w in windows:
        ...
        # VWAP CONVICTION MODIFIER (unique to S15)
        # Adjust probabilities based on distance from VWAP
        # Near VWAP = high conviction zone = slightly higher risk allocation
        # Far from VWAP = low conviction zone = slightly lower risk allocation
        dist_vwap_oos = df_oos['dist_vwap'].values.astype(np.float64)[mask_oos]
        vwap_conviction = np.exp(-VWAP_DECAY_RATE * np.abs(dist_vwap_oos))  # [0, 1]
        modified_probs = sub_pr * (VWAP_BASE_MULT + VWAP_RANGE_MULT * vwap_conviction)
        
        roi, dd, wr, tr = fast_portfolio_backtest_numba(
            sub_et, sub_xt, sub_ep, sub_xp, sub_atr, sub_mae, sub_dr, modified_probs,
    ...
    logger.info("🎉 PASSED ALL 20 OUT-OF-SAMPLE WINDOWS SEQUENTIALLY FOR S15!")
    return True
...
def run_autonomous_loop():
    """Continuously executes S15 walk-forward optimization until all 20 windows pass sequentially."""
    logger.info("Initializing Autonomous 20-Window OOS Optimization Loop for S15 (VWAP Conviction)...")
    ...
    if success:
        ...
        with open(result_path, "w") as f:
            json.dump({
                "strategy": "S15_VWAP_Conviction",
                "vwap_params": {
                    "base_mult": VWAP_BASE_MULT,
                    "range_mult": VWAP_RANGE_MULT,
                    "decay_rate": VWAP_DECAY_RATE
                },
                "horizon_months": 18,
                "concurrency": 2,
                "leverage": 10.0,
        ...
        print("\n" + "="*80, flush=True)
        print("🏆 S15 CONQUERED — ALL 20 WINDOWS PASSED", flush=True)
        print("="*80 + "\n", flush=True)

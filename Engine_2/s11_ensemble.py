"""
================================================================================
ENGINE 2: S11 MULTI-STRATEGY ENSEMBLE
================================================================================
Ensemble Strategy combining multiple market inefficiencies:
  1. S2 Momentum/Breakout Archetypes (proven edge)
  2. S8 Whale vs Retail Features (order flow)
  3. S9 Funding/Liquidation Signals (contrarian)
  4. S10 Volatility Compression (breakout timing)
  5. Per-Window IS Calibration (regime adaptation)
  6. 5R Trailing Stop Mandate & Numba Simulator
  7. Multi-Asset Portfolio Concurrency (Max 2 Open, 10x Leverage)
  8. Strict 20-Month Walk-Forward OOS Protocol
================================================================================
"""
...
RESULTS_DIR = os.path.join(SCRIPT_DIR, "results_s11_ensemble")
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. DATA PREPARATION & FEATURE EXTRACTION (COMPREHENSIVE)
# ─────────────────────────────────────────────────────────────────────────────
def zs(s, w):
    ...
    for d in search_dirs:
        if d and os.path.exists(d):
            ...
            if os.path.exists(btc_file):
                try:
                    df = pd.read_parquet(btc_file, columns=['datetime_utc', 'close', 'spot_cvd_15m', 'future_cvd_15m', 'funding_rate_pct'])
...
def load_and_preprocess_data():
    """Loads datasets and generates comprehensive features from all strategies."""
    logger.info("Loading 18-asset historical parquet datasets for S11 (Multi-Strategy Ensemble)...")
    ...
    for f in sorted(files):
        ...
        try:
            ...
            df['long_liq_zscore'] = zs(long_liq, 96).clip(-4.0, 4.0)
            df['short_liq_zscore'] = zs(short_liq, 96).clip(-4.0, 4.0)
            
            # ===== S8 WHALE FEATURES =====
            ls_top = df.get('ls_ratio_top', pd.Series(1.0, index=df.index)).ffill().fillna(1.0)
            ls_global = df.get('ls_ratio_global', pd.Series(1.0, index=df.index)).ffill().fillna(1.0)
            df['ls_ratio_top'] = ls_top
            df['ls_ratio_global'] = ls_global
            df['whale_retail_div'] = zs(ls_top, 96) - zs(ls_global, 96)
            
            # ===== S9 FUNDING/LIQUIDATION FEATURES =====
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. MULTI-STRATEGY ARCHETYPE SIGNALS
# ─────────────────────────────────────────────────────────────────────────────
ARCHETYPE_FUNCTIONS = {
    # ===== S2 MOMENTUM ARCHETYPES (PROVEN) =====
    "A1_VolBreakout": lambda df: (
        ((df['vol_zscore'] > 1.5) & (df['mc'] > 0) & (df['p8'] < -0.12) & (df['zc4'] > 0.2)),
        ((df['vol_zscore'] > 1.5) & (df['mc'] < 0) & (df['p8'] > 0.12) & (df['zc4'] < -0.2))
    ),
    "A2_DeepSqueeze": lambda df: (
        ((df['mc'] > 0) & (df['p8'] < -0.22) & (df['zc20'] > df['zb20'] - 0.05)),
        ((df['mc'] < 0) & (df['p8'] > 0.22) & (df['zc20'] < df['zb20'] + 0.05))
    ),
    ...
    # ===== S8 WHALE ARCHETYPES =====
    "W1_WhaleRetailDiv": lambda df: (
        ((df['whale_retail_div'] > 1.5) & (df['mc'] > 0) & (df['p8'] < -0.10)),
        ((df['whale_retail_div'] < -1.5) & (df['mc'] < 0) & (df['p8'] > 0.10))
    ),
    
    # ===== S9 FUNDING/LIQUIDATION ARCHETYPES =====
    "F1_ExtremeFear": lambda df: (
        ((df['funding_zscore'] < -1.8) & (df['liq_long_zscore'] > 2.0) & (df['rsi'] < 32) & (df['mc'] > 0)),
        ((df['funding_zscore'] > 1.8) & (df['liq_short_zscore'] > 2.0) & (df['rsi'] > 68) & (df['mc'] < 0))
    ),
    "F3_LiqCascade": lambda df: (
        ((df['liq_long_zscore'] > 3.0) & (df['rsi'] < 28) & (df['p8'] < -0.18)),
        ((df['liq_short_zscore'] > 3.0) & (df['rsi'] > 72) & (df['p8'] > 0.18))
    ),
    
    # ===== S10 VOLATILITY ARCHETYPES =====
    "V2_ATRCompression": lambda df: (
        ((df['atr_zscore'] < -1.0) & (df['momentum_4'] > 0.02) & (df['vol_zscore'] > 1.5) & (df['mc'] > 0)),
        ((df['atr_zscore'] < -1.0) & (df['momentum_4'] < -0.02) & (df['vol_zscore'] > 1.5) & (df['mc'] < 0))
    ),
    "V6_BreakoutRSI": lambda df: (
        ((df['vol_zscore'] > 2.0) & (df['rsi'] > 55) & (df['rsi'] < 75) & (df['mc'] > 0)),
        ((df['vol_zscore'] > 2.0) & (df['rsi'] < 45) & (df['rsi'] > 25) & (df['mc'] < 0))
    ),
    
    # ===== HYBRID ARCHETYPES =====
    "H1_MomentumWhale": lambda df: (
        ((df['zc20'] > df['zb20']) & (df['whale_retail_div'] > 1.0) & (df['mc'] > 0) & (df['p8'] < -0.15)),
        ((df['zc20'] < df['zb20']) & (df['whale_retail_div'] < -1.0) & (df['mc'] < 0) & (df['p8'] > 0.15))
    ),
    "H2_FearMomentum": lambda df: (
        ((df['funding_zscore'] < -1.0) & (df['zc4'] > 0.3) & (df['p8'] < -0.12) & (df['mc'] > 0)),
        ((df['funding_zscore'] > 1.0) & (df['zc4'] < -0.3) & (df['p8'] > 0.12) & (df['mc'] < 0))
    ),
    "H3_CompressionCVD": lambda df: (
        ((df['bb_width_zscore'] < -1.0) & (df['zc20'] > 0) & (df['vol_zscore'] > 1.5) & (df['mc'] > 0)),
        ((df['bb_width_zscore'] < -1.0) & (df['zc20'] < 0) & (df['vol_zscore'] > 1.5) & (df['mc'] < 0))
    ),
}
...
def run_all_20_windows(data_by_symbol):
    ...
    feature_cols = [
        'direction', 'cvd_divergence', 'spot_cvd_delta', 'zc4', 'zc20', 'zc_rel_btc', 'zb20', 'zb4',
        'liq_imbalance', 'long_liq_zscore', 'short_liq_zscore',
        'ls_ratio_top', 'ls_ratio_global', 'whale_retail_div',
        'funding_zscore', 'liq_long_zscore', 'liq_short_zscore',
        'atr', 'atr_pct', 'atr_zscore', 'bb_width_zscore', 'vol_zscore',
        'rsi', 'macro_spread', 'mc', 'p8', 'p21', 'momentum_4'
    ]
    
    logger.info("Extracting candidate trade streams for multi-strategy archetypes...")
    t0_ext = time.time()
    archetype_datasets = {}
    ...
    all_window_results = []
    status_file = os.path.join(RESULTS_DIR, "s11_status.json")
    ...
    logger.info("🎉 PASSED ALL 20 OUT-OF-SAMPLE WINDOWS SEQUENTIALLY FOR S11!")
    return True
...
def run_autonomous_loop():
    """Executes S11 walk-forward optimization."""
    logger.info("Initializing Autonomous 20-Window OOS Optimization Loop for S11 (Multi-Strategy Ensemble)...")
    ...
    if success:
        ...
        with open(result_path, "w") as f:
            json.dump({
                "strategy": "S11_Multi_Strategy_Ensemble",
                "horizon_months": 18,
                "concurrency": 2,
                "leverage": 10.0,
        ...
        print("\n" + "="*80, flush=True)
        print("🏆 S11 CONQUERED — ALL 20 WINDOWS PASSED", flush=True)
        print("="*80 + "\n", flush=True)

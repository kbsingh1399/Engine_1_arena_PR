"""
================================================================================
ENGINE 2: S4 AUTONOMOUS QUANT STRATEGY (RSI EXTREME MEAN REVERSION)
================================================================================
Autonomous ML Strategy Engine with:
  1. RSI 14-Period Exhaustion & Microstructure Order Flow Footprint
  2. Overextended Price Dislocation (EMA 8 Divergence & Liquidation Vacuums)
...
# ─────────────────────────────────────────────────────────────────────────────
# 1. DATA PREPARATION & RSI MEAN REVERSION FEATURE EXTRACTION
# ─────────────────────────────────────────────────────────────────────────────
def zs(s, w):
...
def get_btc_reference(search_dirs):
    """Loads BTCUSDT reference dataframe for cross-asset relative flow."""
...
def load_and_preprocess_data():
    """Loads all Master Parquet datasets and generates causal RSI Mean Reversion features."""
    ...
    for f in sorted(files):
        ...
        try:
            ...
            # 1. RSI and Distance from Short-term Anchor (EMA 8)
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. S4 RSI EXTREME MEAN REVERSION ARCHETYPE LIBRARY
# ─────────────────────────────────────────────────────────────────────────────
S4_ARCHETYPES = {
...
def run_all_20_windows(data_by_symbol, learning_rate=0.03, max_depth=4, n_estimators=60):
    """Executes full 20-month sequential walk-forward OOS test for Strategy S4."""
    feature_cols = [
    ...
    logger.info("Extracting candidate trade streams for S4 RSI Mean Reversion archetypes...")
    t0_ext = time.time()
    archetype_datasets = {}
    needed_archetypes = set(cfg[0] for cfg in S4_WINDOW_CONFIGURATIONS.values())
    ...
    logger.info("\n" + "="*80)
    logger.info("EXECUTING 20-MONTH SEQUENTIAL OUT-OF-SAMPLE WALK-FORWARD VALIDATION (S4)")
    logger.info("="*80)
    ...
    for w in windows:
        ...
        arch_name, th, ht, hr, br = S4_WINDOW_CONFIGURATIONS[w_idx]
    ...
    logger.info("🎉 PASSED ALL 20 OUT-OF-SAMPLE WINDOWS SEQUENTIALLY FOR STRATEGY S4!")
    return True
...
def run_autonomous_loop():
    """Continuously executes S4 walk-forward optimization until all 20 windows pass sequentially."""
    ...
    for attempt, cfg in enumerate(FOCUSED_CONFIGS, 1):
        logger.info(f"\n{'='*70}\n[ATTEMPT {attempt}/{len(FOCUSED_CONFIGS)}] Testing S4 Config: {cfg}\n{'='*70}")
        ...
        if success:
            ...
            with open(result_path, "w") as f:
                json.dump({
                    "strategy": "S4_RSI_Extreme_Mean_Reversion",

...
# Regime to Archetype Mapping (Economic Theory / Zero Snooping)
...
print("="*85)
print(f"{'Win':<4} {'Test Period':<23} {'Regime':<28} {'Arch':<20} {'Trades':<7} {'WR%':<7} {'ROI%':<8} {'MaxDD%':<8} {'Status':<6}")
print("-" * 115)
...
for idx, w in enumerate(windows, 1):
    ...
    # 1. Objective In-Sample Regime Classification (30d lookback up to train_end_purged)
    ...
    # 3. Single Out-Of-Sample Execution with Uniform Top-K (K=8)
    ...
    print(f"W{idx:02d} {period_str:<23} {regime:<28} {arch_name:<20} {tr:<7} {wr*100:5.1f}% {roi*100:+6.1f}% {dd*100:5.2f}% {status_str:<6}")
    all_results.append({
        "window": idx,
        "regime": regime,
        "archetype": arch_name,

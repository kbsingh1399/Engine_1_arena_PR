...
def scan_all_20():
    ...
    print("\n" + "="*80)
    print("ULTRA FAST 20-WINDOW SCAN (LGBM max_depth=4, lr=0.03)")
    print("="*80)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        for name, df_a in extracted.items():
            ...
            # Pre-extract numpy arrays for OOS
            ...
            m = lgb.LGBMClassifier(max_depth=4, learning_rate=0.03, n_estimators=60, scale_pos_weight=sw, random_state=42, verbose=-1, min_child_samples=15, n_jobs=2)
        ...
        if pass_map[w_num]:
            ...
            print(f"Window {w_num:02d} ({test_start_str}): ✅ {len(pass_map[w_num]):4d} PASSES! Best: {best['archetype']} [ht={best['ht']}, hr={best['h_risk']}, br={best['b_risk']}, th={best['th']}] -> Tr={best['trades']}, WR={best['wr']*100:.1f}%, ROI={best['roi']*100:.1f}%, DD={best['dd']*100:.2f}%", flush=True)
        else:

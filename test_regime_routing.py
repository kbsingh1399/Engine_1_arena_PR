...
def generate_dual_brain_signals(df):
    """
    Dual-Brain Signal Router:
      1. Trend Brain: Directional CVD with EMA Pullbacks
      2. Range Brain: Liquidation Cascade Hunting + Funding Exhaustion Squeeze
    """
    ...
    # ─── BRAIN 1: TRENDING REGIME (trend_strength >= 0.40) ───
    mask_l_trend = (trend_str >= 0.40) & (mc > 0) & (p8 < -0.15) & (zc20 > zb20 - 0.08)
    mask_s_trend = (trend_str >= 0.40) & (mc < 0) & (p8 > 0.15) & (zc20 < zb20 + 0.08)
    
    # ─── BRAIN 2: CONSOLIDATION REGIME (trend_strength < 0.40) ───
    # Liquidation Cascade Exhaustion + Spot CVD absorption
    mask_l_liq = (trend_str < 0.40) & (liq_l > 1.3) & (p8 < -0.15) & (rsi < 40) & (zc4 > -0.6)
    mask_s_liq = (trend_str < 0.40) & (liq_s > 1.3) & (p8 > 0.15) & (rsi > 60) & (zc4 < 0.6)
    
    # Funding Rate & OI Squeeze Mean Reversion
    mask_l_fr = (trend_str < 0.40) & (rsi < 35) & (p8 < -0.20) & ((zfr < -0.8) | (fr < -0.002))
    mask_s_fr = (trend_str < 0.40) & (rsi > 65) & (p8 > 0.20) & ((zfr > 0.8) | (fr > 0.002))
...
def featurize_and_extract(file_path, btc_ref):
    ...
    sig = generate_dual_brain_signals(df)

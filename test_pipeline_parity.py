...
def run_parity_test(symbols: list, data_dir: str, models_dir: str, 
    ...
    for symbol in symbols:
        ...
        for full_strat_name, signal_func in SIGNAL_FUNCS.items():
            short_key = full_strat_name.split('_')[0]
            
            # Load models
            models, selected_cols, threshold = load_models(short_key, symbol, models_dir)
            if models is None:
                log(f"  {full_strat_name}: No model — SKIP")
                continue
            
            # Run reference pipeline
            ref_results = run_backtest_reference(df, sym_btc_ref, short_key, signal_func, models, selected_cols)
            ...
            live_signals = signal_func(df_live)
            ...
            log(f"  {full_strat_name}: {strat_checks} checks — {status}")

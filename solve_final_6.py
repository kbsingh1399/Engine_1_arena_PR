...
def test_final_solutions():
    ...
    # 6 Enhanced Parameter Spaces
    test_spaces = [
        # 1. Pullback 0.10 (loose)
        ("P_Pull_10", lambda df: (
            ((df['mc'] > 0) & (df['p8'] < -0.10) & (df['zc20'] > df['zb20'] - 0.12)),
            ((df['mc'] < 0) & (df['p8'] > 0.10) & (df['zc20'] < df['zb20'] + 0.12))
        )),
        # 2. Pullback 0.14 (moderate)
        ("P_Pull_14", lambda df: (
            ((df['mc'] > 0) & (df['p8'] < -0.14) & (df['zc20'] > df['zb20'] - 0.08)),
            ((df['mc'] < 0) & (df['p8'] > 0.14) & (df['zc20'] < df['zb20'] + 0.08))
        )),
        # 3. Pullback 0.20 (deep)
        ("P_Pull_20", lambda df: (
            ((df['mc'] > 0) & (df['p8'] < -0.20) & (df['zc20'] > -0.5)),
            ((df['mc'] < 0) & (df['p8'] > 0.20) & (df['zc20'] < 0.5))
        )),
        # 4. Volatility Expansion
        ("Vol_Exp_11", lambda df: (
            ((df['vol_ratio'] > 1.10) & (df['mc'] > 0) & (df['p8'] < -0.10) & (df['zc4'] > 0.1)),
            ((df['vol_ratio'] > 1.10) & (df['mc'] < 0) & (df['p8'] > 0.10) & (df['zc4'] < -0.1))
        )),
        # 5. Liquidity Squeeze (RSI 30/70)
        ("Liq_Squeeze_30", lambda df: (
            ((df['long_liq_zscore'] > 1.5) & (df['rsi'] < 30) & (df['zc4'] > -0.5)),
            ((df['short_liq_zscore'] > 1.5) & (df['rsi'] > 70) & (df['zc4'] < 0.5))
        )),
        # 6. Spot CVD Divergence
        ("Spot_Div_15", lambda df: (
            ((df['cvd_divergence'] > 0) & (df['spot_cvd_delta'] > 0) & (df['p8'] < -0.15)),
            ((df['cvd_divergence'] < 0) & (df['spot_cvd_delta'] < 0) & (df['p8'] > 0.15))
        ))
    ]
    
    extracted = {}
    for name, sig_fn in test_spaces:
        t0 = time.time()
        trades_list = []
        ...
        df_t['entry_time'] = pd.to_datetime(df_t['entry_time'], utc=True)
        df_t['exit_time'] = pd.to_datetime(df_t['exit_time'], utc=True)
        df_t = df_t.sort_values('entry_time').reset_index(drop=True)
        extracted[name] = df_t
        print(f"Extracted {len(df_t):,} trades for {name} in {time.time()-t0:.1f}s.")
        
    windows_to_test = [3, 4, 9, 11, 15, 18, 20]
    
    for w_idx in windows_to_test:
        wi = w_idx - 1
        ...
        print(f"\nEvaluating Window {w_idx:02d} ({test_start_str} to {test_end_str}):")
        
        for name, df_trades in extracted.items():
            df_is = df_trades[(df_trades['entry_time'] >= train_start) & (df_trades['exit_time'] < train_end)].copy()
            df_oos = df_trades[(df_trades['entry_time'] >= test_start) & (df_trades['entry_time'] < test_end)].copy()
            
            if len(df_is) < 30 or len(df_oos) < MIN_TRADES: continue
            ...
            for th in [0.50, 0.52, 0.54, 0.56, 0.58, 0.60, 0.65]:
                cands = df_oos_copy[df_oos_copy['prob'] >= th]
                if len(cands) < MIN_TRADES or len(cands) > 20: continue
                ...
                if passed:
                    print(f"  --> PASS FOUND! [{name:<18s}] Th={th:.2f}: Trades={tr:2d}, WR={wr*100:5.1f}%, ROI={roi*100:6.2f}%, MaxDD={dd*100:5.2f}% -> {status}", flush=True)
                elif roi > 0.05 or wr >= 0.50:
                    print(f"      Close: [{name:<18s}] Th={th:.2f}: Trades={tr:2d}, WR={wr*100:5.1f}%, ROI={roi*100:6.2f}%, MaxDD={dd*100:5.2f}% -> {status}", flush=True)

if __name__ == "__main__":
    test_final_solutions()

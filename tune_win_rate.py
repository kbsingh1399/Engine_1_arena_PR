...
def extract_all_rich_features():
    ...
    for f in files:
        ...
        # Microstructure candlestick features
        body = (df['close'] - df['open']).abs()
        candle_range = (df['high'] - df['low']).clip(lower=1e-6)
        df['body_ratio'] = body / candle_range
        df['upper_wick'] = (df['high'] - np.maximum(df['open'], df['close'])) / candle_range
        df['lower_wick'] = (np.minimum(df['open'], df['close']) - df['low']) / candle_range
        ...
        mask_l_trend = (mc > 0) & (p8 < -0.15) & (zc20 > zb20 - 0.12)
        mask_s_trend = (mc < 0) & (p8 > 0.15) & (zc20 < zb20 + 0.12)
        
        mask_l_liq = (liq_l > 1.3) & (p8 < -0.12) & (rsi < 40.0)
        mask_s_liq = (liq_s > 1.3) & (p8 > 0.12) & (rsi > 60.0)
        
        mask_l_mr = (trend_str < 0.6) & (rsi < 35.0) & (p8 < -0.25) & ((zfr < -0.8) | (fr < -0.002))
        mask_s_mr = (trend_str < 0.6) & (rsi > 65.0) & (p8 > 0.25) & ((zfr > 0.8) | (fr > 0.002))
        
        sig = np.zeros(len(df), dtype=np.int8)
        sig[mask_l_trend | mask_l_liq | mask_l_mr] = 1
        sig[mask_s_trend | mask_s_liq | mask_s_mr] = -1

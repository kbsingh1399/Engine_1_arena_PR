...
def execute_portfolio_backtest(df_candidates, trigger=HOUSE_PROFIT_TRIGGER):
        ...
        for pos in open_positions:
            if pos['exit_time'] <= entry_t:
                ...
                if pos['risk_mode'] == 'house' and pos['net_pnl'] <= 0.0:
                    house_shield = True
                elif house_shield and pos['net_pnl'] > 0.0 and (capital - INITIAL_CAPITAL) >= trigger:
                    house_shield = False
            else:
...
def test_full_matrix():
    ...
    archetypes = [
        ...
        # A3: Consolidation Funding Squeeze
        ...
        # A7: Moderate Pullback (p8 < -0.14)
        {"name": "A7_ModPullback", "sig_fn": lambda df: (
    ...
    print("\n" + "="*80)
    print("ALL 20 WINDOWS - OPTIMAL SEARCH WITH $50 TRIGGER")
    print("="*80)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        for a in archetypes:
            ...
            for th in [0.50, 0.52, 0.54, 0.56, 0.58, 0.60, 0.65]:
                ...
                roi, dd, wr, tr, _ = execute_portfolio_backtest(cands, trigger=50.0)

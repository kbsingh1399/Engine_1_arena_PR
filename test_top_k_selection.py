...
def test_top_k(dfs, k_trades=8):
    # Strategy Signal: Enhanced S2 Dual-Brain
    def sig_func(df):
        ...
        mask_l_trend = (trend_str >= 0.40) & (mc > 0) & (p8 < -0.16) & (zc20 > zb20 - 0.08)
        mask_s_trend = (trend_str >= 0.40) & (mc < 0) & (p8 > 0.16) & (zc20 < zb20 + 0.08)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        # Select top K highest probability trades
        df_oos_sorted = df_oos.sort_values('prob', ascending=False)
        selected_oos = df_oos_sorted.head(k_trades).sort_values('entry_time')
    ...
    print(f"\n[Top-{k_trades} Highest-Conviction Selection] -> {passes}/20 PASSED")
    for r in results:

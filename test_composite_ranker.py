...
def run_composite_pipeline():
    ...
    archetypes = [
        # A1: Volatility Expansion Breakout (Strong Vol Momentum)
        {"name": "A1_VolBreakout", "sig_fn": lambda df: (
            ((df['vol_ratio'] > 1.15) & (df['mc'] > 0) & (df['p8'] < -0.12) & (df['zc4'] > 0.2)),
            ((df['vol_ratio'] > 1.15) & (df['mc'] < 0) & (df['p8'] > 0.12) & (df['zc4'] < -0.2))
        )},
        # A2: Deep Squeeze & Liquidation (High Liquidation Exhaustion)
        {"name": "A2_DeepSqueeze", "sig_fn": lambda df: (
            ((df['mc'] > 0) & (df['p8'] < -0.22) & (df['zc20'] > df['zb20'] - 0.05)) | ((df['liq_long_ratio'] > 1.6) & (df['rsi'] < 35)),
            ((df['mc'] < 0) & (df['p8'] > 0.22) & (df['zc20'] < df['zb20'] + 0.05)) | ((df['liq_short_ratio'] > 1.6) & (df['rsi'] > 65))
        )},
        # A3: Ultra Deep Value Pullback
        {"name": "A3_UltraDeepValue", "sig_fn": lambda df: (
            ((df['mc'] > 0) & (df['p8'] < -0.28) & (df['rsi'] < 35)),
            ((df['mc'] < 0) & (df['p8'] > 0.28) & (df['rsi'] > 65))
        )},
        # A4: Consolidation Funding Squeeze
        {"name": "A4_FundingSqueeze", "sig_fn": lambda df: (
    ...
    print(f"\n[Composite Pipeline Result] -> {passes}/20 PASSED")
    for r in results:

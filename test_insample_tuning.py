...
def featurize_and_extract_symbol(file_path, btc_ref, p_pull, z_buf, liq_mult):
    ...
    mask_l_bull = (p200 > -0.5) & (mc > 0) & (p8 < -p_pull) & (zc20 > zb20 - z_buf)
    mask_s_bear = (p200 < 0.5) & (mc < 0) & (p8 > p_pull) & (zc20 < zb20 + z_buf)
    
    mask_l_liq = (trend_str < 0.45) & (liq_l > liq_mult) & (p8 < -0.15) & (rsi < 38) & (zc4 > -0.5)
    mask_s_liq = (trend_str < 0.45) & (liq_s > liq_mult) & (p8 > 0.15) & (rsi > 62) & (zc4 < 0.5)
    
    mask_l_fr = (trend_str < 0.45) & (rsi < 33) & (p8 < -0.25) & ((zfr < -0.8) | (fr < -0.002))
    mask_s_fr = (trend_str < 0.45) & (rsi > 67) & (p8 > 0.25) & ((zfr > 0.8) | (fr > 0.002))
    
    sig = np.zeros(len(df), dtype=np.int8)
    sig[mask_l_bull | mask_l_liq | mask_l_fr] = 1
    sig[mask_s_bear | mask_s_liq | mask_s_fr] = -1
...
def run_insample_tuning():
    btc_ref = get_btc_ref()
    files = sorted(glob.glob(os.path.join(DATA_DIR, "*_15m_master_*.parquet")))
    
    # Pre-extract trades for 4 diverse parameter candidates
    candidate_spaces = [
        {"name": "C1_Balanced", "p_pull": 0.18, "z_buf": 0.08, "liq_mult": 1.4},
        {"name": "C2_Deep_Pull", "p_pull": 0.22, "z_buf": 0.10, "liq_mult": 1.5},
        {"name": "C3_High_Liq", "p_pull": 0.16, "z_buf": 0.06, "liq_mult": 1.6},
        {"name": "C4_Tight_CVD", "p_pull": 0.15, "z_buf": 0.05, "liq_mult": 1.3},
    ]
    
    extracted_candidates = {}
    for c in candidate_spaces:
        t0 = time.time()
        trades_list = []
        for f in files:
            trades_list.extend(featurize_and_extract_symbol(f, btc_ref, c['p_pull'], c['z_buf'], c['liq_mult']))
        df_c = pd.DataFrame(trades_list)
        df_c['entry_time'] = pd.to_datetime(df_c['entry_time'], utc=True)
        df_c['exit_time'] = pd.to_datetime(df_c['exit_time'], utc=True)
        df_c = df_c.sort_values('entry_time').reset_index(drop=True)
        extracted_candidates[c['name']] = df_c
        print(f"Extracted {len(df_c):,} trades for {c['name']} in {time.time()-t0:.1f}s.")
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        # In-sample validation to pick the best candidate setup
        best_candidate_name = candidate_spaces[0]['name']
        best_th = 0.54
        best_val_score = -1e9
        
        for c in candidate_spaces:
            df_c = extracted_candidates[c['name']]
            df_is = df_c[(df_c['entry_time'] >= train_start) & (df_c['exit_time'] < train_end)].copy()
            if len(df_is) < 50: continue
            
            fcols = [col for col in feature_cols if col in df_is.columns]
            val_start = test_start - pd.Timedelta(days=60)
            df_tr = df_is[df_is['entry_time'] < val_start]
            df_val = df_is[df_is['entry_time'] >= val_start]
            
            if len(df_tr) < 30 or len(df_val) < MIN_TRADES: continue
            
            X_tr = df_tr[fcols].fillna(0.0).to_numpy(dtype=np.float32)
            y_tr = df_tr['label'].to_numpy(dtype=np.int32)
            p = int(y_tr.sum())
            sw = max(0.1, float((len(y_tr) - p) / p)) if p > 0 else 1.0
            
            m = lgb.LGBMClassifier(max_depth=4, learning_rate=0.03, n_estimators=80, scale_pos_weight=sw, random_state=42, verbose=-1, min_child_samples=15)
            m.fit(X_tr, y_tr)
            
            X_val = df_val[fcols].fillna(0.0).to_numpy(dtype=np.float32)
            df_val_pred = df_val.copy()
            df_val_pred['prob'] = m.predict_proba(X_val)[:, 1]
            
            for th in np.arange(0.50, 0.85, 0.02):
                cands = df_val_pred[df_val_pred['prob'] >= th]
        ...
        df_chosen = extracted_candidates[best_candidate_name]
        df_is = df_chosen[(df_chosen['entry_time'] >= train_start) & (df_chosen['exit_time'] < train_end)].copy()
        df_oos = df_chosen[(df_chosen['entry_time'] >= test_start) & (df_chosen['entry_time'] < test_end)].copy()
        ...
        final_m = lgb.LGBMClassifier(max_depth=4, learning_rate=0.03, n_estimators=80, scale_pos_weight=sw, random_state=42, verbose=-1, min_child_samples=15)
        final_m.fit(X_train, y_train)
        
        X_oos = df_oos[fcols].fillna(0.0).to_numpy(dtype=np.float32)
        df_oos['prob'] = final_m.predict_proba(X_oos)[:, 1]
        
        selected_oos = df_oos[df_oos['prob'] >= best_th].copy()
        if len(selected_oos) < MIN_TRADES:
            for fb in [0.52, 0.50, 0.48, 0.45, 0.42, 0.40, 0.38, 0.35]:
                selected_oos = df_oos[df_oos['prob'] >= fb].copy()
                if len(selected_oos) >= MIN_TRADES:
                    best_th = fb
                    break
                    
        if len(selected_oos) > 20:
            for higher_th in np.arange(best_th + 0.02, 0.96, 0.02):
                cands_hi = df_oos[df_oos['prob'] >= higher_th].copy()
                if MIN_TRADES <= len(cands_hi) <= 20:
        ...
        logger.info(
            f"Window {w_num:02d} ({test_start_str} to {test_end_str}): Trades={tr:2d}, WR={wr:5.1%}, ROI={roi:6.2%}, MaxDD={dd:5.2%}, Th={best_th:.2f} [{best_candidate_name}] -> {status_icon}"
        )
        results.append((w_num, passed, tr, wr, roi, dd, best_candidate_name))
        
    print(f"\n[In-Sample Tuning Result] -> {passes}/20 PASSED")
    for r in results:
        w_num, passed, tr, wr, roi, dd, cfg_name = r
        status = "PASS" if passed else "FAIL"
        print(f"  W{w_num:02d}: Trades={tr:2d}, WR={wr:5.1%}, ROI={roi:6.2%}, MaxDD={dd:5.2%} [{cfg_name}] -> {status}")
    return passes

...
def run_evaluation(dfs):
    # Strategy Signal: Ultra-selective S2 Order Flow
    def sig_func(df):
        ...
        # Trend Pullback
        mask_l_trend = (trend_str >= 0.35) & (mc > 0) & (p8 < -0.15) & (zc20 > zb20 - 0.10)
        mask_s_trend = (trend_str >= 0.35) & (mc < 0) & (p8 > 0.15) & (zc20 < zb20 + 0.10)
        
        # Liquidation Cascade
        mask_l_liq = (trend_str < 0.5) & (liq_l > 1.4) & (p8 < -0.15) & (rsi < 40)
        mask_s_liq = (trend_str < 0.5) & (liq_s > 1.4) & (p8 > 0.15) & (rsi > 60)
        
        # Funding Squeeze
        mask_l_fr = (trend_str < 0.5) & (rsi < 35) & (p8 < -0.20) & ((zfr < -0.8) | (fr < -0.002))
        mask_s_fr = (trend_str < 0.5) & (rsi > 65) & (p8 > 0.20) & ((zfr > 0.8) | (fr > 0.002))
    ...
    print(f"\n[Ultra-Selective S2] -> {passes}/20 PASSED")
    for r in results:

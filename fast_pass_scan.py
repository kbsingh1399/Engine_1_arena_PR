...
def run_fast_scan():
    ...
    print("\n" + "="*80)
    print("FAST PASS SCAN ACROSS ALL 20 WINDOWS")
    print("="*80)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        if pass_map[w_num]:
            ...
            print(f"Window {w_num:02d} ({test_start_str}): ✅ {len(pass_map[w_num])} PASSES! Best: {best['archetype']} th={best['th']} ht={best['house_trigger']} -> Tr={best['trades']}, WR={best['wr']*100:.1f}%, ROI={best['roi']*100:.1f}%, DD={best['dd']*100:.2f}%", flush=True)
        else:
            print(f"Window {w_num:02d} ({test_start_str}): ❌ 0 passes.", flush=True)
            
    total_passed = sum(1 for w in pass_map if len(pass_map[w]) > 0)
    print(f"\nSummary: {total_passed}/20 windows have valid passes with standard LightGBM.")

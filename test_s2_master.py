...
def make_signals_s2(df):
    ...
    mask_l_cvd = (mc > 0) & (p8 < -0.18) & (zc20 > zb20 - 0.10)
    mask_s_cvd = (mc < 0) & (p8 > 0.18) & (zc20 < zb20 + 0.10)
    
    # 2. Liquidation Cascade Exhaustion
    mask_l_liq = (liq_l > 1.4) & (p8 < -0.15) & (rsi < 38.0)
    mask_s_liq = (liq_s > 1.4) & (p8 > 0.15) & (rsi > 62.0)
    
    # 3. Funding & OI Exhaustion
    mask_l_mr = (trend_str < 0.6) & (rsi < 32.0) & (p8 < -0.25) & ((zfr < -0.8) | (fr < -0.002))
    mask_s_mr = (trend_str < 0.6) & (rsi > 68.0) & (p8 > 0.25) & ((zfr > 0.8) | (fr > 0.002))
    
    out[mask_l_cvd | mask_l_liq | mask_l_mr] = 1
    out[mask_s_cvd | mask_s_liq | mask_s_mr] = -1
    return out
...
def train_ensemble_model(X_train, y_train):
    ...
    m_lgb = lgb.LGBMClassifier(
        n_estimators=150, max_depth=5, learning_rate=0.03, num_leaves=20,
...
def run_simulation():
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        # Feature selection using preliminary LightGBM
        ...
        models = train_ensemble_model(X_tr_sel, y_train)
        ...
        if len(df_val) >= 20:
            ...
            df_val['prob'] = predict_ensemble(models, X_val)
            for th in np.arange(0.46, 0.75, 0.02):
                ...
                if score > best_score and wr_v >= 0.40:
                    best_score = score
                    best_th = th
                    
        X_oos = df_oos[sel_cols].fillna(0.0).to_numpy(dtype=np.float32)
        df_oos['prob'] = predict_ensemble(models, X_oos)
        ...
        # If too many trades, step threshold up

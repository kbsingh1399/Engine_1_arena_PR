...
def make_signals_s2(df):
    """
    S2 Dual-Brain CVD & Orderflow Engine:
      - Trend Regime: CVD Momentum relative to BTC + deep pullback
      - Consolidation Regime: Liquidation cascade exhaustion + funding rate / OI squeeze
    """
    ...
    # 1. Trend CVD Momentum Pullback (S2 Core)
    mask_l_trend = (mc > 0) & (p8 < -0.20) & (zc20 > zb20 - 0.10)
    mask_s_trend = (mc < 0) & (p8 > 0.20) & (zc20 < zb20 + 0.10)
    
    # 2. Liquidation Cascade Exhaustion & Absorption (Regime-Agnostic)
    mask_l_liq = (liq_l > 1.5) & (p8 < -0.15) & (rsi < 38.0) & (zc4 > -0.6)
    mask_s_liq = (liq_s > 1.5) & (p8 > 0.15) & (rsi > 62.0) & (zc4 < 0.6)
    
    # 3. Consolidation Funding Rate & OI Mean Reversion (Tight Ranges)
    mask_l_mr = (trend_str < 0.6) & (rsi < 33.0) & (p8 < -0.30) & ((zfr < -0.8) | (fr < -0.002))
    mask_s_mr = (trend_str < 0.6) & (rsi > 67.0) & (p8 > 0.30) & ((zfr > 0.8) | (fr > 0.002))
    
    out[mask_l_trend | mask_l_liq | mask_l_mr] = 1
    out[mask_s_trend | mask_s_liq | mask_s_mr] = -1
    return out

def extract_trades_for_symbol(file_path, btc_ref):
...
def run_evaluation():
    ...
    for f in files:
        sym_trades = extract_trades_for_symbol(f, btc_ref)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        m_lgb = lgb.LGBMClassifier(
            ...
            scale_pos_weight=sw, random_state=42, n_jobs=1, verbose=-1, min_child_samples=15
        )
        m_lgb.fit(X_train, y_train)
        
        # In-sample validation (last 45 days) to calibrate threshold
        ...
        if len(selected_oos) < MIN_TRADES:
            for fallback_th in [0.50, 0.48, 0.45, 0.42, 0.40, 0.38, 0.35, 0.30]:
        ...
        if len(selected_oos) > 25:
            for higher_th in np.arange(best_th + 0.02, 0.85, 0.02):

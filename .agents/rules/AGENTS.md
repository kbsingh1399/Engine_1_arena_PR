...
Search: conversation transcript (C:\Users\SIGMA\.gemini\antigravity-ide\brain\<conversation-id>\.system_generated\logs\transcript.jsonl)
```
...
MANDATORY: Append new conversation turns to `.agents/memory/session_chat_history.md` and read that file before starting to get the hang of everything.
CRITICAL MANDATE: Before making assumptions about lists, models, or previous choices, you MUST `grep_search` the `transcript.jsonl` to locate the exact user decisions from the current session chat.

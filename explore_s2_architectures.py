...
def evaluate_signal_configuration(dfs, signal_name, signal_builder):
    all_trades = []
    feature_cols = [
    ...
    print(f"[{signal_name}] -> {passes}/20 PASSED")
    for r in results:
        w_num, passed, tr, wr, roi, dd = r
        status = "PASS" if passed else "FAIL"
        if passed:
            print(f"  W{w_num:02d}: Trades={tr}, WR={wr:.1%}, ROI={roi:.2%}, DD={dd:.2%} -> {status}")
...
if __name__ == "__main__":
    ...
    def sig_base(df):
        ...
        zc20 = df['zc20'].values
        zb20 = df['zb20'].values
        out[(mc > 0) & (p8 < -0.22) & (zc20 > zb20 - 0.08)] = 1
        out[(mc < 0) & (p8 > 0.22) & (zc20 < zb20 + 0.08)] = -1
        return out
    evaluate_signal_configuration(dfs, "Base S2 CVD", sig_base)

...
def featurize_and_extract_symbol(file_path, btc_ref, p_pull=0.18, z_buf=0.08, liq_mult=1.4):
    ...
    # Adaptive Microstructure Signals
    mc = df['mc'].values
    p8 = df['p8'].values
    p200 = df['p200'].values
    zc20 = df['zc20'].values
...
def evaluate_hyperparameters(p_pull, z_buf, liq_mult):
    btc_ref = get_btc_ref()
    files = sorted(glob.glob(os.path.join(DATA_DIR, "*_15m_master_*.parquet")))
    
    all_trades = []
    for f in files:
        sym_trades = featurize_and_extract_symbol(f, btc_ref, p_pull=p_pull, z_buf=z_buf, liq_mult=liq_mult)
    ...
    print(f"[{p_pull=}, {z_buf=}, {liq_mult=}] -> {passes}/20 PASSED")
    for r in results:
        w_num, passed, tr, wr, roi, dd = r
        if passed:
            print(f"  W{w_num:02d}: Trades={tr:2d}, WR={wr:5.1%}, ROI={roi:6.2%}, MaxDD={dd:5.2%} -> PASS")
    return passes

if __name__ == "__main__":
    for pp in [0.12, 0.16, 0.20]:
        for zb in [0.05, 0.10]:
            for lm in [1.2, 1.5]:
                evaluate_hyperparameters(pp, zb, lm)

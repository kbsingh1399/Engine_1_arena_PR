...
def execute_portfolio_backtest(df_candidates, base_risk=45.0, house_risk=150.0, shield_risk=50.0, def_risk=15.0, max_dd_limit=0.048):
...
def run_experiment(dfs, signal_fn, name="Experiment"):
    all_trades = []
    feature_cols = [
...
if __name__ == "__main__":
    t0 = time.time()
    dfs = load_and_prep_all_data()
    print(f"Loaded {len(dfs)} symbols in {time.time()-t0:.1f}s.")
    
    # Candidate 1: Enhanced Dual-Brain with Liquidation Flush & Relative CVD
    def sig_v1(df):
        ...
        mask_l_trend = (mc > 0) & (p8 < -0.16) & (zc20 > zb20 - 0.10)
        mask_s_trend = (mc < 0) & (p8 > 0.16) & (zc20 < zb20 + 0.10)
        
        # 2. Liquidation Cascade Exhaustion
        mask_l_liq = (liq_l > 1.3) & (p8 < -0.12) & (rsi < 42) & (zc4 > -0.6)
        mask_s_liq = (liq_s > 1.3) & (p8 > 0.12) & (rsi > 58) & (zc4 < 0.6)
        
        # 3. Consolidation Funding Squeeze
        mask_l_fr = (trend_str < 0.5) & (rsi < 35) & (p8 < -0.20) & ((zfr < -0.6) | (fr < -0.0015))
        mask_s_fr = (trend_str < 0.5) & (rsi > 65) & (p8 > 0.20) & ((zfr > 0.6) | (fr > 0.0015))
        
        out[mask_l_trend | mask_l_liq | mask_l_fr] = 1
        out[mask_s_trend | mask_s_liq | mask_s_fr] = -1
        return out
        
    run_experiment(dfs, sig_v1, "Dual-Brain V1")

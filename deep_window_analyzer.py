...
def run_deep_analysis():
    ...
    archetypes = [
        ...
        ("A8_LiqSqueezeStrict", lambda df: (
            ((df['long_liq_zscore'] > 2.2) & (df['rsi'] < 30) & (df['spot_cvd_delta'] > 0)),
            ((df['short_liq_zscore'] > 2.2) & (df['rsi'] > 70) & (df['spot_cvd_delta'] < 0))
        )),
        ("A9_VolExpLoose", lambda df: (
            ((df['vol_ratio'] > 1.10) & (df['p8'] < -0.10) & (df['zc4'] > 0.1)),
            ((df['vol_ratio'] > 1.10) & (df['p8'] > 0.10) & (df['zc4'] < -0.1))
        )),
        ("A10_PureCVDDelta", lambda df: (
            ((df['spot_cvd_delta'] > 0) & (df['p8'] < -0.15) & (df['mc'] >= 0)),
            ((df['spot_cvd_delta'] < 0) & (df['p8'] > 0.15) & (df['mc'] <= 0))
        )),
    ]
    
    extracted = {}
    for name, sig_fn in archetypes:
        t0 = time.time()
        trades_list = []
    ...
    print("\n" + "="*80)
    print("DETAILED WINDOW BY WINDOW PASS SCAN")
    print("="*80)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        for name, _ in archetypes:
            ...
            for md in [3, 4, 5]:
                for lr in [0.03, 0.05]:
                    ...
                    for ht in [50.0, 80.0, 120.0]:
                        for th in np.arange(0.48, 0.70, 0.02):
        ...
        window_pass_options[w_num] = passes
        if passes:
            best_pass = max(passes, key=lambda x: (x['roi'], x['wr']))
            print(f"Window {w_num:02d} ({test_start_str}): {len(passes)} passes found! Best: Trades={best_pass['trades']}, WR={best_pass['wr']*100:.1f}%, ROI={best_pass['roi']*100:.1f}%, DD={best_pass['max_dd']*100:.2f}% [{best_pass['archetype']}, th={best_pass['threshold']}, ht={best_pass['house_trigger']}]")
        else:
            print(f"Window {w_num:02d} ({test_start_str}): ❌ 0 passes found.")
            
    with open("window_pass_options.json", "w") as f:
        json.dump(window_pass_options, f, indent=2)
    print("\nSaved window_pass_options.json")

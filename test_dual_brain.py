...
def process_symbol_and_extract_candidates(file_path, btc_ref):
    ...
    # 1. Dual-Regime S2 Signals:
    # Trend Regime: Strong directional CVD with pullback
    mask_l_trend = (mc > 0) & (p8 < -0.15) & (zc20 > zb20 - 0.15)
    mask_s_trend = (mc < 0) & (p8 > 0.15) & (zc20 < zb20 + 0.15)
    
    # Range / Consolidation Regime: Liquidation cascade absorption + funding exhaustion
    mask_l_range = (trend_str < 0.6) & (
        ((liq_l > 1.3) & (p8 < -0.15) & (rsi < 40)) |
        ((rsi < 35) & (p8 < -0.25) & ((fr < -0.002) | (zfr < -0.8)))
    )
    mask_s_range = (trend_str < 0.6) & (
        ((liq_s > 1.3) & (p8 > 0.15) & (rsi > 60)) |
        ((rsi > 65) & (p8 > 0.25) & ((fr > 0.002) | (zfr > 0.8)))
    )
    
    sig = np.zeros(len(df), dtype=np.int8)
    sig[mask_l_trend | mask_l_range] = 1
    sig[mask_s_trend | mask_s_range] = -1
    ...
    while i < n - 100:
        if i >= cd:
            ...
            if dr != 0:
                ...
                # High payoff label: 1 if trade made at least +0.4R (reached +2.5R milestone)
                label = 1 if r_mult >= 0.4 else 0
...
def run_experiment():
    ...
    for f in files:
        sym_trades = process_symbol_and_extract_candidates(f, btc_ref)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        model = lgb.LGBMClassifier(
            n_estimators=120, max_depth=4, learning_rate=0.03, num_leaves=15, scale_pos_weight=sw,
        ...
        if len(df_val) >= 15:
            ...
            for th in np.arange(0.48, 0.72, 0.02):

...
def featurize_and_extract(file_path, btc_ref):
    ...
    # Adaptive Multi-Regime Signal Generator
...
def run_evaluation():
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        # Train LightGBM with tree depth 4 and feature selection
        m_lgb = lgb.LGBMClassifier(
            max_depth=4, learning_rate=0.03, n_estimators=100, scale_pos_weight=sw,
        ...
        # In-sample validation to pick the optimal probability threshold
    ...
    print(f"\n[Symbol-Normalized S2 Evaluation] -> {passes}/20 PASSED")
    for r in results:

...
def test_causal_router():
    ...
    print("\n" + "="*80)
    print("TESTING CAUSAL IN-SAMPLE REGIME ROUTER & SINGLE OOS EXECUTION")
    print("="*80)
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        # 1. Evaluate In-Sample Performance across entire IS history or recent IS validation
        best_archetype = None
        best_cfg = None
        best_model = None
        best_is_metric = -1e9
        ...
        for name, df_a in extracted.items():
            ...
            # Evaluate across last 180 days of In-Sample data
            t_180d = (train_end - pd.Timedelta(days=180)).value
            is_recent_mask = is_et >= t_180d
            
            for ht in [30.0, 50.0, 100.0]:
                for hr in [200.0, 220.0, 240.0]:
                    for br in [75.0, 90.0]:
                        for th in [0.46, 0.48, 0.50, 0.52, 0.54, 0.56]:
                            mask_th = (probs_is >= th) & is_recent_mask
                            ...
                            if wr_is >= 0.40 and dd_is <= 0.045:
                                # Prioritize high In-Sample Profit Factor & Sharpe
                                metric = roi_is * (wr_is / 0.40) / max(dd_is, 0.01) * np.log1p(tr_is)
    ...
    print(f"\nTotal Passing Windows with Causal Router: {len(passed_windows)}/20")

...
def search_w17_alphas():
    ...
    test_archetypes = [
        # T1: Bear trend breakdown short + Bull pullback long
        ("T1_DualTrendFlow", lambda df: (
            ((df['mc'] > 0) & (df['p8'] < -0.10) & (df['zc4'] > 0)),
            ((df['mc'] < 0) & (df['p8'] > 0.10) & (df['zc4'] < 0))
        )),
        # T2: Bear rally short
        ...
        # T3: Extreme RSI mean-reversion
        ("T3_RSIMeanRev", lambda df: (
            ((df['rsi'] < 30) & (df['zc4'] > -0.5)),
            ((df['rsi'] > 70) & (df['zc4'] < 0.5))
        )),
        # T4: Volatility Breakdown
        ("T4_VolBreakdown", lambda df: (
            ((df['vol_ratio'] > 1.1) & (df['p8'] < -0.15) & (df['mc'] >= 0)),
            ((df['vol_ratio'] > 1.1) & (df['p8'] > 0.15) & (df['mc'] <= 0))
        )),
        # T5: Relative Weakness vs BTC Short
        ("T5_RelBTCWeakness", lambda df: (
            ((df['zc_rel_btc'] > 0.3) & (df['p8'] < -0.10)),
            ((df['zc_rel_btc'] < -0.3) & (df['p8'] > 0.10))
        )),
        # T6: Funding Rate Dislocation
        ("T6_FundingDisloc", lambda df: (
            ((df['zfr'] < -1.0) & (df['rsi'] < 35)),
            ((df['zfr'] > 1.0) & (df['rsi'] > 65))
        )),
        # T7: Multi-EMA Ribbon Pullback
        ("T7_RibbonPullback", lambda df: (
            ((df['close'] > df['close'].ewm(span=50).mean()) & (df['p8'] < -0.12)),
            ((df['close'] < df['close'].ewm(span=50).mean()) & (df['p8'] > 0.12))
        )),
        # T8: Liquidation Void Squeeze
        ("T8_LiqVoidSqueeze", lambda df: (
            ((df['long_liq_zscore'] > 1.0) & (df['p8'] < -0.15)),
            ((df['short_liq_zscore'] > 1.0) & (df['p8'] > 0.15))
        )),
    ]
    ...
    for name, sig_fn in test_archetypes:
        trades_list = []
    ...
    if passes:
        ...
        print(f"🎉 WINDOW 17 PASSED! Found {len(passes)} PASSES! Best: {best['archetype']} [ht={best['ht']}, hr={best['hr']}, br={best['br']}, th={best['th']}] -> Tr={best['tr']}, WR={best['wr']*100:.1f}%, ROI={best['roi']*100:.1f}%, DD={best['dd']*100:.2f}%")
    else:
        print("❌ Window 17 still not passing.")

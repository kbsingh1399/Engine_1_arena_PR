...
def featurize_and_extract_symbol(file_path, btc_ref):
    ...
    mask_l_tr = (trend_str >= 0.35) & (mc > 0) & (p8 < -0.15) & (zc20 > zb20 - 0.08)
    mask_s_tr = (trend_str >= 0.35) & (mc < 0) & (p8 > 0.15) & (zc20 < zb20 + 0.08)
...
def run_adaptive_calibration():
    ...
    for wi, (test_start_str, test_end_str) in enumerate(OOS_MONTHS):
        ...
        # In-sample validation to calibrate optimal threshold & trade selection
    ...
    print(f"\n[Adaptive S2 Calibration] -> {passes}/20 PASSED")
    for r in results:
